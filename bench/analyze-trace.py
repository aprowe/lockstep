...
import gzip, json, sys, re, glob, os
sys.stdout.reconfigure(encoding='utf-8')

# CLI arg or newest Trace-*.json.gz in this directory
if len(sys.argv) > 1:
    TRACE = sys.argv[1]
else:
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = sorted(glob.glob(os.path.join(here, 'Trace-*.json.gz')))
    if not candidates:
        print('No Trace-*.json.gz found in', here, file=sys.stderr); sys.exit(1)
    TRACE = candidates[-1]
