...
The timeline is a horizontally scrubbing region of the UI made up of stacked **tracks** (rows). Each track surfaces a particular _kind_ of object — anchors, scenes, regions/clips, the warp connector layer — and each kind responds to pointer, keyboard, and wheel input in a coherent way.
...
- pointerdown in an **empty area** → arms a _lasso_ gesture (does not activate yet).
- pointerdown on an **object** (anchor / region body / region edge / warp line / scene) → arms a potential _drag_ of that object.
...
- **Input-space drag** snaps to:
    - scene cuts
    - clip / region edges
    - _No_ BPM grid snapping in input space.
...
| Cursor _inside_ an existing region 60–70 + scene at 80 | 70–80 (behaves as if playhead is just past the region's out) |
