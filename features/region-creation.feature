Feature: Region Creation
  ...
  Scenario: New region from the timeline is centred on the cursor position
    ...
    Then the region spans from 58 to 62 seconds

  Scenario: New region from the region list is centred on the playhead
    Given the current viewport span is 40 seconds
    And the video duration is 120 seconds
