...
function usage(msg: string): never {
    ...
    console.error("usage:");
    console.error(
        "  npx tsx scripts/screenshot-local.ts --pr <N> --instructions <json|@file> [--comment <text>] [--dry-run]",
    );
    process.exit(2);
}
...
try {
    ...
    const exists = sh("git", ["ls-remote", "--exit-code", "--heads", remoteUrl, branch], {
        allowFail: true,
    });
    if (exists.status === 0) {
    ...
    sh("git", ["commit", "-q", "-m", `Screenshots for PR #${args.pr} (local ${ts})`], {
        cwd: scratch,
    });
...
sh(
    ...
    [
        ...
        "--method",
        "POST",
        `repos/${owner}/${repo}/issues/${args.pr}/comments`,
        "--input",
        "-",
    ],
    {
