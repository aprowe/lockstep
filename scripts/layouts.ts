...
function parseLayoutDoc(doc: unknown, fileRel: string, docIndex: number): LayoutEntry {
    ...
    const groups: LayoutGroup[] = groupsRaw.map((groupRaw, gi) => {
        ...
        if (Array.isArray(groupRaw)) {
            ...
            Array.isArray((groupRaw as any).items)
        ) {
...
function parseAll(): Record<string, LayoutEntry> {
    ...
    const files = walkDir(LAYOUTS_DIR).filter(
        (f) => f.endsWith(".layout.yaml") || f.endsWith(".layout.yml"),
    );
    for (const file of files) {
        ...
        for (const entry of entries) {
            if (registry[entry.id]) {
                throw new Error(
                    `Duplicate layout ID "${entry.id}" in ${entry.file} (already in ${registry[entry.id].file})`,
                );
            }
...
function runParse() {
    ...
    const totalItems = Object.values(layouts).reduce(
        (sum, l) => sum + l.groups.reduce((s, g) => s + g.items.length, 0),
        0,
    );
    ...
    for (const l of Object.values(layouts)) {
        ...
        console.log(
            `  ${c.cyan(l.id)}  ${c.gray(`(${l.groups.length} groups, ${itemCount} items)`)}`,
        );
        l.groups.forEach((group, gi) => {
            for (const item of group.items) {
                const extra = item.attrs ? c.gray(` [${Object.keys(item.attrs).join(", ")}]`) : "";
                console.log(
                    `    ${c.gray("-")} ${item.id.padEnd(24)} ${c.gray(item.label)}${extra}`,
                );
            }
...
function runCheck() {
    runParse();
    const registry = JSON.parse(readFileSync(REGISTRY, "utf8")).layouts as Record<
        string,
        LayoutEntry
    >;

    const testFiles = walkDir(TESTS_DIR).filter(
