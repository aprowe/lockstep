...
for (const tool of ["ffmpeg", "ffprobe"]) {
    ...
    try {
        ...
        execSync(isWin ? `move "${extracted}" "${dest}"` : `mv "${extracted}" "${dest}"`, {
        ...
        try {
            execSync(isWin ? `if exist "${zip}" del "${zip}"` : `rm -f "${zip}"`, {
                shell: isWin ? "cmd" : "/bin/sh",
            });
            execSync(isWin ? `if exist "${tmp}" rmdir /s /q "${tmp}"` : `rm -rf "${tmp}"`, {
                shell: isWin ? "cmd" : "/bin/sh",
            });
        } catch {
            /* best-effort cleanup */
...
function setupRife() {
    ...
    console.log(
        `  ↓  rife-ncnn-vulkan  →  ${url}  (~430MB, bundles all models; we keep only the binary + ${RIFE_MODEL})`,
    );
