...
var cmd = args.find(function (a) { return !a.startsWith('-'); });
...
// ─── color helpers ────────────────────────────────────────────────────────────
var c = {
    reset: function (s) { return NO_COLOR ? s : "\u001B[0m".concat(s, "\u001B[0m"); },
    bold: function (s) { return NO_COLOR ? s : "\u001B[1m".concat(s, "\u001B[0m"); },
    dim: function (s) { return NO_COLOR ? s : "\u001B[2m".concat(s, "\u001B[0m"); },
    green: function (s) { return NO_COLOR ? s : "\u001B[32m".concat(s, "\u001B[0m"); },
    red: function (s) { return NO_COLOR ? s : "\u001B[31m".concat(s, "\u001B[0m"); },
    yellow: function (s) { return NO_COLOR ? s : "\u001B[33m".concat(s, "\u001B[0m"); },
    cyan: function (s) { return NO_COLOR ? s : "\u001B[36m".concat(s, "\u001B[0m"); },
    gray: function (s) { return NO_COLOR ? s : "\u001B[90m".concat(s, "\u001B[0m"); },
};
// ─── shared helpers ───────────────────────────────────────────────────────────
...
function normalizeSteps(lines) {
    return lines
        .map(function (l) { return l.replace(/^\s*(Given|When|Then|And|But)\s*:?\s+/i, '').trim(); })
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
...
function parseFeatureFile(content, relPath) {
    ...
    var exampleRows = [];
    var flush = function () {
        if (!scenarioTitle || steps.length === 0)
            return;
        var hashInput = __spreadArray([
            scenarioTitle.toLowerCase().trim(),
            normalizeSteps(steps)
        ], exampleRows, true).join('\n');
        var id = "".concat(toSlug(featureTitle), "::").concat(shortHash(hashInput));
        if (behaviors[id])
            console.warn("  WARN: ID collision in ".concat(relPath, ": ").concat(id));
        behaviors[id] = { feature: featureTitle, scenario: scenarioTitle, isOutline: isOutline, steps: steps.map(function (s) { return s.trim(); }), file: relPath, line: scenarioLine };
        steps = [];
        scenarioTitle = '';
        isOutline = false;
        inExamples = false;
    ...
    for (var _i = 0, _a = lines.entries(); _i < _a.length; _i++) {
        ...
        if (inExamples) {
            if (t.startsWith('|'))
                exampleRows.push(t.replace(/\s+/g, ' '));
            continue;
        }
        if (STEP_RE.test(t))
            steps.push(t);
...
function runParse() {
    ...
    for (var _i = 0, _a = findFiles(FEATURES_DIR, '.feature'); _i < _a.length; _i++) {
        ...
        for (var _b = 0, _c = Object.entries(parseFeatureFile(content, rel)); _b < _c.length; _b++) {
            var _d = _c[_b], id = _d[0], entry = _d[1];
            if (all[id]) {
...
function runCoverage(print) {
    ...
    var coverage = new Map(__spreadArray([], expectedIds, true).map(function (id) { return [id, []]; }));
    for (var _i = 0, testRefs_1 = testRefs; _i < testRefs_1.length; _i++) {
        var ref = testRefs_1[_i];
        if (coverage.has(ref.id))
            coverage.get(ref.id).push(ref);
    }
    var missing = __spreadArray([], expectedIds, true).filter(function (id) { return coverage.get(id).length === 0; });
    var orphans = testRefs.filter(function (ref) { return !expectedIds.has(ref.id); });
    var covered = __spreadArray([], expectedIds, true).filter(function (id) { return coverage.get(id).length > 0; });
    var pct = expectedIds.size === 0 ? 100 : Math.round((covered.length / expectedIds.size) * 100);
    var result = { total: expectedIds.size, covered: covered.length, percentage: pct, missing: missing, orphans: orphans };
    (0, node_fs_1.mkdirSync)(GENERATED, { recursive: true });
    ...
    console.log("".concat(c.bold("COVERED  (".concat(covered.length, "/").concat(expectedIds.size, ")")), "\n").concat(c.gray(light)));
    ...
    if (missing.length === 0) {
        ...
        for (var _d = 0, missing_1 = missing; _d < missing_1.length; _d++) {
            ...
            console.log("    ".concat(c.gray(behaviors[id].scenario)));
            console.log("    ".concat(c.gray('spec:'), " ").concat(behaviors[id].file, ":").concat(behaviors[id].line));
        }
    }
    ...
    var summary = "Coverage: ".concat(covered.length, "/").concat(expectedIds.size, " (").concat(pct, "%)  |  ").concat(orphans.length, " orphaned ref(s)");
