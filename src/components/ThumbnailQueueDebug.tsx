  ...
  return (
    <div className="thumb-debug">
      <div className="thumb-debug__header">
        ...
        <button type="button" className="thumb-debug__close" onClick={onClose} title="Close">×</button>
      </div>
      {!fileHash ? (
        ...
        <>
          <div className="thumb-debug__summary">
            <div><span>workers</span><b>{stats.workers_running}</b></div>
            <div><span>ready</span><b>{stats.total_ready}/{stats.max_cached_frames}</b></div>
            <div><span>in-flight</span><b>{stats.total_in_flight}</b></div>
            <div><span>max frame</span><b>{stats.max_frame}</b></div>
          </div>
          <table className="thumb-debug__table">
            ...
            <tbody>
              {stats.tiers.map(t => (
                <tr key={t.name} className={t.total === 0 ? 'thumb-debug__row--empty' : ''}>
                  <td>{t.name}</td>
                  <td>{t.total}</td>
                  <td>{t.pending}</td>
                  <td>{t.in_flight}</td>
