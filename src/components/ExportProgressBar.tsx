...
export default function ExportProgressBar() {
  ...
  const title = ep.totalJobs > 1
    ? `${ep.label} (${ep.jobIdx + 1}/${ep.totalJobs})`
    : ep.label || 'Export'
  ...
  return (
    ...
    >
      <div className="export-mini__head">
        <span className="export-mini__label" title={title}>{title}</span>
        <span className="export-mini__pct">
          {ep.status === 'processing' ? `${pct}%` : ep.status === 'done' ? 'Done' : 'Error'}
        </span>
        {ep.status !== 'processing' && (
          <span
            ...
            role="button"
            aria-label="Dismiss"
            onClick={e => { e.stopPropagation(); dispatch(resetExportProgress()) }}
          >✕</span>
        )}
      </div>
      ...
      {ep.message && ep.status === 'processing' && (
        <div className="export-mini__msg" title={ep.message}>{ep.message}</div>
      )}
      {ep.error && ep.status === 'error' && (
        <div className="export-mini__msg export-mini__msg--error" title={ep.error}>{ep.error}</div>
      )}
    </button>
  )
}
