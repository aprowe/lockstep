...
export default function ExportDialog({
    ...
    const detectedSceneCuts = useAppSelector(
    ...
    useEffect(() => {
        ...
        if (open && status !== "processing") {
            ...
            const preSelected = (selectedClipIds ?? []).filter((id) =>
    ...
    const beatsForRegion = (
        r: { inBeatTime: number; outBeatTime: number; bpm: number } | null,
    ): number | null => {
        if (!r) return null;
                ...
                const outputPath = await new Promise<string>((resolve, reject) => {
                    listenWarpProgress((payload) => {
                        ...
                        if (msg) {
                            ...
                            setLogLines((prev) =>
                                prev.length > 0 && prev[prev.length - 1] === msg
                                    ? prev
                                    : [...prev, msg],
                            );
                        }
    ...
    const handleSaveOne = async (idx: number) => {
        ...
        try {
            ...
            if (warpData)
                await writeMarkerSidecar(savedPath, warpData, {
                    videoName: originalName,
                    exportFolder: destFolder,
                });
    ...
    const handleSaveAll = async () => {
        ...
        try {
            ...
            for (let i = 0; i < outputPaths.length; i++) {
                ...
                if (destFolder) {
                    savedPath = await saveToFolder({
                        source_path: outputPaths[i],
                        dest_folder: destFolder,
                        file_name: fileName,
                    });
                } else {
                    savedPath = await saveOutput({
                        source_path: outputPaths[i],
        ...
        >
            <div className="export-dialog">
                ...
                <div className="export-dialog__body">
                        ...
                        <div className="export-dialog__clip-list">
                            {regions.map((r) => (
                                <label key={r.id} className="export-dialog__clip-item">
                                    ...
                                    <span className="export-dialog__clip-bpm">
                                        {Math.round(r.bpm)}
                                    </span>
                                </label>
                            ))}
                        ...
                        <div className="export-dialog__output">
                            <div className="export-dialog__row">
                                ...
                                <input
                                    ...
                                    value={destFolder ?? ""}
                                    onChange={(e) =>
                                        reduxDispatch(setLastExportFolder(e.target.value || null))
                                    }
                                ...
                                {lastExportFolder && (
                                    <button
                                        className="export-dialog__folder-clear"
                                        onClick={() => reduxDispatch(setLastExportFolder(null))}
                                        title="Reset to video folder"
                                    >
                                        ✕
                                    </button>
                                )}
                        ...
                        <div className="export-dialog__options">
                            ...
                            <div className="export-dialog__norm-row">
                                <label className="export-dialog__check">
                                    <input
                                        type="checkbox"
                                        checked={normalizeBpm}
                                        onChange={(e) => setNormalizeBpm(e.target.checked)}
                                    />
                                    Normalize to
                                </label>
                                {normalizeBpm && (
                                ...
                                {normalizeBpm && (
                                    <span className="export-dialog__norm-label">BPM</span>
                                )}
                            </div>
                            <div className="export-dialog__interp">
                                ...
                                {interpolateFrames && (
                                    <div
                                        className="export-dialog__interp-panel"
                                        aria-label="Interpolation Options"
                                    >
                                        <label className="export-dialog__interp-field">
                                            <span className="export-dialog__interp-label">
                                                Method
                                            </span>
                                            <select
                                                ...
                                                value={interpMethod}
                                                onChange={(e) =>
                                                    setInterpMethod(e.target.value as InterpMethod)
                                                }
                                            >
                                        ...
                                        <label className="export-dialog__interp-field">
                                            ...
                                            <input
                                                ...
                                                value={interpFps}
                                                onChange={(e) =>
                                                    setInterpFps(Number(e.target.value))
                                                }
                                            />
                                        </label>
                                    </div>
                    ...
                    {logLines.length > 0 && (
                        <div className="export-dialog__log" ref={logRef} aria-label="Export Log">
                            {logLines.map((line, i) => (
                                <div key={i} className="export-dialog__log-line">
                                    {line}
                                </div>
                            ))}
                        </div>
                ...
                <div className="export-dialog__footer">
                    {status === "error" && (
                        <span className="export-dialog__error" title={error ?? ""}>
                            {error}
                        </span>
                    )}
                    ...
                    {status === "processing" ? (
                        <div className="export-dialog__progress-wrap">
                            <div className="export-dialog__progress-text">
                                <span className="export-dialog__progress-pct">
                                    {Math.round(
                                        ((currentJobIdx + progress) / Math.max(totalJobs, 1)) * 100,
                                    )}
                                    %
                                </span>
                                {currentMessage && (
                                    <span
                                        className="export-dialog__progress-msg"
                            ...
                            <div className="export-dialog__progress">
                                <div
                                    ...
                                    style={{
                                        width: `${((currentJobIdx + progress) / Math.max(totalJobs, 1)) * 100}%`,
                                    }}
                                />
                            </div>
                        </div>
                        ...
                        <div className="export-dialog__results">
                            {outputPaths.length === 1 ? (
                                <div className="export-dialog__results-row">
                                    <button
                                        className="export-dialog__save"
                                        onClick={() => handleSaveOne(0)}
                                ...
                                <>
                                    <div className="export-dialog__results-row">
                                        <button
                                            ...
                                            disabled={saving || savedCount >= outputPaths.length}
                                        >
                                            {saving
                                                ? "…"
                                                : savedCount >= outputPaths.length
                                                  ? `✓ All Saved`
                                                  : `Save All (${outputPaths.length})`}
                                        </button>
                                        {savedFolder && (
                                            <button
                                                className="export-dialog__open-folder"
                        ...
                        <button
                            ...
                            onClick={process}
                            disabled={
                                !canProcess || (mode === "selected" && selectedRegionIds.size === 0)
                            }
                        >
                            {mode === "all"
