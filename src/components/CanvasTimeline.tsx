...
import {
  MINIMAP_H, TRI_HALF, TRI_H, FONT,
  buildLayout,
  type LayoutTrack,
} from '../timeline/layout'
import {
  timeLayers, barsLayers,
} from '../timeline/ruler'
import {
  createHitListBuilder, hitAt as hitAtPure,
  type HitListBuilder,
} from '../timeline/hitTest'
import { createTimelineController } from '../timeline/controller'
import type {
  Snapshot, Intent, PointerEventLike, WheelEventLike, KeyEventLike,
} from '../timeline/types'
...
export default function CanvasTimeline(props: CanvasTimelineProps) {
  ...
  const clearHits = () => { hitsBuilderRef.current = createHitListBuilder() }
  ...
  function draw() {
    ...
    function layerScenes() {
      ...
      for (const t of p.scenes) {
        ...
        const isSel    = (p.selectedSceneTimes?.has(t) ?? false) || (lassoSceneTimes?.has(t) ?? false)
    ...
    // ── Clip regions (helper) ─────────────────────────────
    function drawRegions(tr: LayoutTrack | undefined, regions: RegionBlock[], isOutput: boolean, draggable = true) {
      if (!tr) return
      ...
      for (const r of sorted) {
        ...
        const offLeft  = x1 < 0
        const offRight = x2 > W
        const bx1 = cx1 + 0.5, bx2 = cx2 - 0.5
        const by1 = tr.y + 2.5, by2 = tr.y + tr.h - 2.5
        ctx.beginPath()
        ctx.moveTo(bx1, by1); ctx.lineTo(bx2, by1)
        ctx.moveTo(bx1, by2); ctx.lineTo(bx2, by2)
        if (!offLeft)  { ctx.moveTo(bx1, by1); ctx.lineTo(bx1, by2) }
        if (!offRight) { ctx.moveTo(bx2, by1); ctx.lineTo(bx2, by2) }
        ctx.stroke()
        if (offLeft || offRight) {
          const lw = ctx.lineWidth
          ctx.setLineDash([3, 4])
          ctx.beginPath()
          if (offLeft)  { ctx.moveTo(bx1 + lw / 2, by1); ctx.lineTo(bx1 + lw / 2, by2) }
          if (offRight) { ctx.moveTo(bx2 - lw / 2, by1); ctx.lineTo(bx2 - lw / 2, by2) }
          ctx.stroke()
          ctx.setLineDash([])
        }
        ...
        if (cw > 20 && r.label && !isOutput) {
          ctx.fillStyle = pal.fg1; ctx.globalAlpha = 0.9
          setFont(10, true)
    ...
    function layerThroughLines() {
      ...
      for (const pair of anchorPairs) {
        ...
        const origSel = p.selectedOrigAnchorIds.has(pair.id) || (lassoOrigAnchorIds?.has(pair.id) ?? false)
        const beatSel = p.selectedBeatAnchorIds.has(pair.id) || (lassoBeatAnchorIds?.has(pair.id) ?? false)
        const sel = origSel || beatSel
        if (!hov && !sel && !alwaysAnchors) continue
        ctx.strokeStyle = hov ? pal.throughHover : sel ? 'hsla(195,85%,75%,0.7)' : pal.throughLine
      ...
      if (alwaysScenes || (p.selectedSceneTimes && p.selectedSceneTimes.size > 0) || (lassoSceneTimes?.size ?? 0) > 0) {
        if (inp) {
          for (const t of p.scenes) {
            const isSel  = (p.selectedSceneTimes?.has(t) ?? false) || (lassoSceneTimes?.has(t) ?? false)
    ...
    function layerRegionEnvelopes() {
      ...
      for (let ri = 0; ri < n; ri++) {
        const rIn = regions[ri], rO = rOut[ri]
    ...
    function drawAnchorIn(x: number, tr: LayoutTrack, hov: boolean, sel: boolean) {
      ...
      ctx.strokeStyle = anchorCol; ctx.lineWidth = hov ? 2 : 1.5
    ...
    function drawAnchorOut(x: number, tr: LayoutTrack, hov: boolean, linked: boolean, sel = false) {
      if (x < -TRI_HALF - 2 || x > W + TRI_HALF + 2) return
      const anchorCol = hov ? pal.markerHover : sel ? 'hsl(195,85%,78%)' : pal.markerColor
      ctx.strokeStyle = anchorCol; ctx.lineWidth = hov ? 2 : 1.5
      ...
      ctx.closePath()
      if (linked) {
        ctx.fillStyle = sel && !hov ? 'hsla(195,85%,70%,0.35)' : anchorCol; ctx.fill()
      } else {
    ...
    function layerAnchorMarkers() {
      ...
      if (trIn) {
        for (const a of anchors) {
          ...
          const sel = p.selectedOrigAnchorIds.has(a.id) || (lassoOrigAnchorIds?.has(a.id) ?? false)
      ...
      if (trOut) {
        for (const a of beatAnchors) {
          ...
          const selBeat = p.selectedBeatAnchorIds.has(a.id) || (lassoBeatAnchorIds?.has(a.id) ?? false)
    ...
    function layerBeatRuler() {
      ...
      for (const layer of barsLayers(ppb, bpb)) {
        ...
        const tkClr = layer.styleKey === 'bar' ? pal.barTick : layer.styleKey === 'beat' ? pal.beatTick : pal.subTick
        const gdClr = layer.styleKey === 'bar' ? pal.gridBar : layer.styleKey === 'beat' ? pal.gridBeat : 'rgba(0,0,0,0)'
    ...
    function layerSpeedStrip() {
      ...
      const hasActiveRegion =
        ...
        beatIn  !== undefined && beatOut  !== undefined &&
        clipOut > clipIn + 1e-6 && beatOut > beatIn + 1e-6
      ...
      for (let i = 0; i < allMarkers.length - 1; i++) {
        ...
        if (cx2 - cx1 > 28) {
          ctx.fillStyle = pal.fg3; setFont(9, false)
    ...
    function layerPlayhead() {
      ...
      function vline(x: number, y1: number, y2: number, glow: boolean) {
        ...
        if (glow) {
          ctx.strokeStyle = pal.playheadGlow; ctx.lineWidth = 3
          ctx.beginPath(); ctx.moveTo(px, y1); ctx.lineTo(px, y2); ctx.stroke()
        }
        ctx.strokeStyle = pal.playhead; ctx.lineWidth = 1
        ctx.beginPath(); ctx.moveTo(px, y1); ctx.lineTo(px, y2); ctx.stroke()
      }
  ...
  useEffect(() => {
    ...
    tracks,
    props.playhead, props.beatPlayhead,
    ...
    gestHoverAnchorId, gestHoverRegionId, gestHoverSceneTime, gestHoverWarpLineId,
  ])
  ...
  function applyIntents(intents: Intent[]) {
    ...
    for (const i of intents) {
      switch (i.kind) {
        ...
        case 'clipsSelectionChange': p.onClipsSelectionChange?.(i.clipinIds, i.clipoutIds); break
        case 'scenesSelectionChange': p.onScenesSelectionChange?.(i.times); break
        case 'connectorSelectionChange': p.onConnectorSelectionChange?.(i.origIds, i.beatIds); break
        ...
        case 'pubHoveredWarpLine': gesture.setHoveredWarpLine(i.id); break
        case 'thumbnailHover': setThumbnailHover(i.payload); break
        case 'cursor':
