...
import { buildSegments, snapAllToBeat } from "../utils/quantize";
...
export default function WarpView({
    ...
    selectedClipinIds,
    selectedClipoutIds,
    onClipsSelectionChange,
    selectedSceneTimes,
    onScenesSelectionChange,
    userSceneTimes,
    onTimelineDelete,
    onTimelineDeselect,
    onClipOverlayResize,
    ...
    const clipLockedBeats = useMemo(() => {
        ...
        if (activeRegion.lockedBeats && activeRegion.lockedBeats > 0)
            return activeRegion.lockedBeats;
    ...
    const { scopedOrig, scopedBeat } = useMemo(() => {
        ...
        const filteredBeat = filteredOrig.map(
            (oa) => sortedBeat.find((ba) => ba.id === oa.id) ?? { id: oa.id, time: oa.time },
        );
        ...
        if (
            clipOut !== undefined &&
            (augO.length === 0 || clipOut - augO[augO.length - 1].time > EPS)
        ) {
        ...
        (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    if (!Array.isArray(data.origAnchors) || !Array.isArray(data.beatAnchors))
                        return;
                    ...
                    if (data.beatZeroAnchorTime != null) {
                        const match = newBeat.find(
                            (a) => Math.abs(a.time - data.beatZeroAnchorTime) < 0.001,
                        );
                    ...
                    ...(onSendToNewRegion
                        ? [
                              { separator: true as const },
                              {
                                  ...
                                  action: () => {
                                      const selOrig = origAnchors.filter((a) =>
                                          targetIds.has(a.id),
                                      );
    ...
    const handleThinAnchorAdd = useCallback(
        (time: number) => {
            const clamped = Math.max(0, Math.min(duration, time));
            dispatch(
                setOrigAnchorsFromTimeline([...origAnchors, { id: newAnchorId(), time: clamped }]),
            );
        },
        [duration, origAnchors, dispatch],
    );

    const handleThinAnchorDelete = useCallback(
        (id: number) => {
    ...
    const handleThinAnchorSelect = useCallback(
    ...
    const handleThinBeatAnchorDelete = useCallback(
        (id: number) => {
            dispatch(removeAnchors([id]));
        },
        [dispatch],
    ...
    const handleThinBeatAnchorSelect = useCallback(
        (id: number, additive: boolean) => {
            if (additive) {
