...
import { setListFilterMode, setListSelection, setListThumbnailMode, type ListId, type ListThumbnailMode } from '../../store/slices/listsSlice'
...
export default function ListPanel<T extends ListItem>({
  listId, items, activeId, onActivate, onDelete,
  renderRow, prefixRows, emptyHint, headerActions, subHeader,
  selectedIdsOverride, onSelectionChangeOverride,
  hideClipFilter, clipFilterDisabled,
}: ListPanelProps<T>) {
  ...
  const buildCtx = useCallback((item: T): RowContext => {
    const isActive = activeId != null && activeId === item.id
    const isSelected = selectedSet.has(item.id)
    const thumbFrame = item.thumbnailTime != null && fps > 0
  ...
  }, [activeId, selectedSet, thumbnailMode, thumbPaths, fps, handleRowClick, setHover, multiSelectMode, toggleSelection])
  ...
  return (
    <div
      ...
      tabIndex={0}
      onKeyDown={e => { if (handleKeyDown(e)) e.preventDefault() }}
