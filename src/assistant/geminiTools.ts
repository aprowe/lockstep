        ...
        segments: {
            ...
            items: {
                ...
                properties: {
                    ...
                    label: {
                        type: "string",
                        description: "Short description of what is in this segment.",
                    },
                    confidence: {
                        type: "number",
                        description: "Confidence 0..1 that the segment matches the query.",
                    },
                },
...
export const geminiExtension: Extension = {
    ...
    name: "Gemini video",
    description:
        "Whole-video understanding via Google Gemini. Native temporal " +
        "reasoning — much cheaper than per-frame analysis for long clips.",
    tools: [
        {
            name: "analyze_video",
            description:
                "Ask Gemini a free-form question about the entire active video and get a text answer. Good for descriptions, summaries, or anything where structure isn't required. The video is uploaded once and cached on Gemini's side for ~48 hours.",
            input_schema: {
                type: "object",
                properties: {
                    prompt: {
                        type: "string",
                        description: "The question to ask Gemini about the video.",
                    },
                },
                required: ["prompt"],
        ...
        },
        {
            ...
            description:
                'Ask Gemini to locate every segment of the active video matching a query (e.g. "scenes with horses", "shots of someone dancing", "talking-head sections"). Returns structured {start, end, label, confidence} ranges. Combine with `add_region` to materialize the matches as named regions.',
            input_schema: {
                type: "object",
                properties: {
                    query: {
                    ...
                    max_segments: {
                        ...
                        minimum: 1,
                        maximum: 50,
                        description: "Hard cap on segments returned. Default 20.",
                    },
                },
                required: ["query"],
                additionalProperties: false,
