...
const extractFrameTool: ToolHandler = async (args: any, { store, log }) => {
  ...
  return {
    blocks: [
      ...
      { type: 'image', source: { type: 'base64', media_type: frame.mime_type, data: frame.base64 } },
    ],
  }
}
...
export const coreExtension: Extension = {
  ...
  tools: [
    {
      name: 'get_video',
      description: 'Return info about the currently loaded video (path, duration, fps, fileHash). If nothing is loaded, returns a notice instead of erroring.',
    ...
    {
      name: 'list_regions',
      description: 'List named regions (sub-clips) on the active video. Each has its own in/out and BPM. Pass `query` to filter by case-insensitive name substring.',
      input_schema: {
        type: 'object',
    ...
    {
      name: 'list_markers',
      description: 'List beat-marker anchors on the active video, sorted by time. Markers are points; regions are spans.',
      input_schema: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
      name: 'list_scenes',
    ...
    {
      name: 'extract_frame',
      description: 'Extract a JPEG frame at the given time from the active video and return it as image content. Use this to identify what is visually in a frame (e.g. animals, people, objects).',
      input_schema: {
        type: 'object',
        properties: {
          time:      { type: 'number', description: 'Seconds from the start of the video.' },
          max_width: { type: 'integer', minimum: 0, description: 'Scale longest side to this many pixels (0 = source). Default 640.' },
        },
        required: ['time'],
        additionalProperties: false,
      },
    ...
    {
      name: 'add_region',
      description: 'Create a named region (sub-clip) on the active video. Beat zero is always the inPoint. Inherits the active BPM if `bpm` is omitted.',
      input_schema: {
        type: 'object',
        properties: {
          name:     { type: 'string' },
    ...
    {
      name: 'add_scene_cut',
      description: 'Add a user-placed scene cut on the active video. User cuts always survive the min-gap filter.',
      input_schema: {
        type: 'object',
        properties: { time: { type: 'number' } },
        required: ['time'],
    ...
    {
      name: 'seek_to',
      description: 'Persist the playhead position. Mostly useful for narrating "look here" — does not actually move the player.',
      input_schema: {
        type: 'object',
        properties: { time: { type: 'number' } },
        required: ['time'],
    ...
    {
      name: 'open_video',
      description: 'Load a video file by absolute path into the app, replacing any currently loaded video.',
      input_schema: {
        type: 'object',
