...
async function initiateResumable(
  ...
  if (!res.ok) throw new Error(`Gemini upload init ${res.status}: ${truncate(await safeText(res), 300)}`)
...
async function uploadAndFinalize(
  ...
  if (!res.ok) throw new Error(`Gemini upload ${res.status}: ${truncate(await safeText(res), 300)}`)
...
async function getFile(apiKey: string, name: string, signal?: AbortSignal): Promise<GeminiFile> {
  const res = await fetch(
    `${FILES_BASE}/v1beta/${name}?key=${encodeURIComponent(apiKey)}`,
    { signal },
  )
  if (!res.ok) throw new Error(`Gemini get-file ${res.status}: ${truncate(await safeText(res), 200)}`)
...
function writeCache(entry: CachedUpload): void {
  ...
  } catch { /* best-effort cache; not fatal */ }
}
