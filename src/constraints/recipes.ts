...
export function initClip(clipInId: EntityId, clipOutId: EntityId): Op[] {
  return [
    { kind: OpKind.AddConstraint, constraint: {
      kind: ConstraintKind.DirectedPair,
      from: clipInId, to: clipOutId, mode: PairMode.Translate,
    ...
    { kind: OpKind.AddConstraint, constraint: {
      ...
      clipId: clipInId, min: 0.1, mode: PreserveMode.Shift,
    }},
    { kind: OpKind.AddConstraint, constraint: {
      kind: ConstraintKind.Clamp, entityId: clipInId, field: 'in',  min: 0,
    }},
    { kind: OpKind.AddConstraint, constraint: {
      kind: ConstraintKind.Clamp, entityId: clipInId, field: 'out', min: 0,
    }},
  ]
}
...
export function clearLasso(groupTag: string): Op {
  return {
    kind: OpKind.RemoveConstraint,
    predicate: c =>
      c.kind === ConstraintKind.TranslateGroup && c.tag === `lasso:${groupTag}`,
  }
}
...
export function setBpm(
  ...
  const derived = state.constraints.find(c =>
    c.kind === ConstraintKind.Derived &&
    c.tag  === `bpm:${clipId}`,
...
export function setLockedBeats(
  clipId: EntityId,
  newBeats: number,
  state: State,
): Op[] {
...
export function setActiveClip(activeId: EntityId | null): Op[] {
  return [
    ...
    { kind: OpKind.AddConstraint, constraint: {
      ...
      filterKind: EntityKind.Clip, role: Role.Active, activeId,
    }},
  ]
}
...
export function setBeatZero(anchorId: EntityId | null): Op[] {
  return [
    ...
    { kind: OpKind.AddConstraint, constraint: {
      kind: ConstraintKind.SingleOfKind,
      filterKind: EntityKind.Anchor, role: Role.BeatZero, activeId: anchorId,
    }},
  ]
}
...
export function warpLineEnd(anchorInId: EntityId): Op {
  return {
    kind: OpKind.RemoveConstraint,
    predicate: c => c.kind === ConstraintKind.TranslateGroup && c.tag === `warpline:${anchorInId}`,
  }
}
...
export function snapToSiblings(
  ...
  for (const rule of index.rules) {
    ...
    if (rule.target === 'grid') { includeGrid = true; continue }
