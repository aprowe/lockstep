...
function preDragEdgeValue(h: EdgeHandle, ctx: { preDrag: { regions: ReadonlyArray<{ id: string; inPoint: number; outPoint: number; inBeatTime: number; outBeatTime: number }> } }): number | undefined {
  ...
  whileDragging: (handle, ctx, state) => {
    ...
    if (handle.space === 'beat') {
      ...
      if (lockActive) {
        ...
        if (region) {
          const inner = innerBeatAnchorIds(ctx.preDrag.beatAnchors, region.inBeatTime, region.outBeatTime)
          if (inner.length > 0) {
            cs.push({
