...
import { runConstraintPipeline, extractDragCtxFromSlice, type PipelineSlice } from "./pipeline";
...
export function extractSliceForPipeline(state: RootState): PipelineSlice {
    ...
    return {
        warp: {
            ...
            beatAnchors: (s.warp?.beatAnchors ?? []).map((a) => ({
                id: a.id,
                time: a.time,
 ...
 */
export function dispatchPipelined(dispatch: AppDispatch, getState: () => RootState, op: Op): void {
    const preState = getState();
    const preSlice = extractSliceForPipeline(preState);
    const dragCtx = extractDragCtxFromSlice(
        preState as Parameters<typeof extractDragCtxFromSlice>[0],
    );
 ...
 */
export function beginReplayFrame(dispatch: AppDispatch, getState: () => RootState): void {
    const state = getState();
    const preDrag = state.drag?.preDrag;
    if (!preDrag) return;

    // Region position resets — restore any drifted field to its preDrag value.
    const regionDiffs: Record<
        string,
        Partial<{
            inPoint: number;
            outPoint: number;
