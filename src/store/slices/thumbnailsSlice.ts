...
export const { setThumbnail, clearForHash, setStripFrames, setHoverFrames } =
...
export function selectStripFramesFor(
    ...
    selector = createSelector(
        [
            (s: { thumbnails: ThumbnailsState }) =>
                hash ? s.thumbnails.stripFramesBySource[hash] : undefined,
        ],
        (bySource) => {
...
export function selectThumbnailPathsFor(
    ...
    return (state) => {
        ...
        return (
            state.thumbnails.pathsByHashAndFrame[hash] ??
            (EMPTY_FRAME_PATHS as Record<number, string>)
        );
    };
}
