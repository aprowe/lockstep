...
export const applyRemoveAnchors =
    (ids: number[]) => (dispatch: AppDispatch, getState: () => RootState) => {
...
export const applyOrigAnchorsFromTimeline =
    (nextOrigAnchors: readonly Anchor[]) => (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        const removedIds = state.warp.origAnchors
        ...
        for (const a of added) {
            ...
            for (const op of addAnchorOps(a.id, a.time, a.time))
...
export const applyBeatAnchorsFromTimeline =
    (nextBeatAnchors: readonly Anchor[]) => (dispatch: AppDispatch, getState: () => RootState) => {
        const state = getState();
...
export const applyClearAnchors = () => (dispatch: AppDispatch, getState: () => RootState) => {
    const state = getState();
...
export const applySetOrigAnchors =
    (anchors: readonly Anchor[]) => (dispatch: AppDispatch, getState: () => RootState) => {
        const state = getState();
    ...
    (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (next.inPoint !== sliceR.inPoint) {
            dispatchPipelined(
                dispatch,
                getState,
                setRegionInEdgeOp(payload.id, "in", next.inPoint),
            );
        }
        if (next.outPoint !== sliceR.outPoint) {
            dispatchPipelined(
                dispatch,
                getState,
                setRegionInEdgeOp(payload.id, "out", next.outPoint),
            );
        }
    };
...
export const applyUpdateRegionBeatTimes =
    ...
    (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatchPipelined(
            dispatch,
            getState,
            setRegionOutEdgeOp(payload.id, "in", payload.inBeatTime),
        );
        dispatchPipelined(
            dispatch,
            getState,
            setRegionOutEdgeOp(payload.id, "out", payload.outBeatTime),
        );
        dispatch(
            _syncRegionPositions({
                [payload.id]: {
...
export const applyResetRegionBoundary =
    (payload: { id: string }) => (dispatch: AppDispatch, getState: () => RootState) => {
        const state = getState();
        const sliceR = state.region.regions.find((r) => r.id === payload.id);
        if (!sliceR) return;
        ...
        dispatchPipelined(
            dispatch,
            getState,
            setRegionOutEdgeOp(payload.id, "out", sliceR.outPoint),
        );
        dispatch(
            _syncRegionPositions({
                [payload.id]: {
...
export const applyLinkingEvent =
    ...
    (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        dispatchPipelined(
            dispatch,
            getState,
            setRegionOutEdgeOp(payload.id, "in", result.inBeatTime),
        );
        dispatchPipelined(
            dispatch,
            getState,
            setRegionOutEdgeOp(payload.id, "out", result.outBeatTime),
        );
    ...
    (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if (payload.stretch) {
            ...
            dispatchPipelined(
                dispatch,
                getState,
                setRegionOutEdgeOp(payload.id, "out", inBeatTime + newLength),
            );
...
export const applyAddRegion =
    (region: Region) => (dispatch: AppDispatch, getState: () => RootState) => {
