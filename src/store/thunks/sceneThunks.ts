...
>("scene/detect", async ({ path, threshold, window }, { dispatch, getState }) => {
    await dispatch(ensureSceneListener());
    const state = getState() as RootState;
    const effectiveThreshold = threshold ?? state.scene.thresholdByPath[path] ?? 10;
...
export const cancelSceneDetectionThunk = createAsyncThunk<void, void>("scene/cancel", async () => {
