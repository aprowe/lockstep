...
export default function VideoInfoPanel() {
    ...
    return (
        <div className="vinfo">
            <div className="vinfo__name" title={video.originalName}>
                {video.originalName}
            </div>
            {dir && (
                <div className="vinfo__dir" title={video.path}>
                    {dir}
                </div>
            )}
            ...
            <div className="vinfo__grid">
                ...
                <Row
                    label="Frames"
                    value={frameCount(video.duration, video.fps).toLocaleString()}
                />
                <Row label="Frame rate" value={`${video.fps.toFixed(2)} fps`} />
                {resolution && <Row label="Resolution" value={resolution} />}
                {video.videoCodec && <Row label="Video codec" value={video.videoCodec} />}
                {video.container && <Row label="Container" value={video.container} />}
                {video.fileSize != null && (
                ...
                {audioParts.length > 0 && <Row label="Audio" value={audioParts.join(" · ")} />}
            </div>
        </div>
    );
}
