...
export function timeLayers(pps: number): TickLayer[] {
    ...
    if (sub * pps >= 6)
        layers.push({ spacingUnit: sub, styleKey: "sub", tickHeight: 5, skipModulo: ratio });
    layers.push({
        spacingUnit: major,
        styleKey: "bar",
        isMajor: true,
...
export function barsLayers(ppb: number, bpb: number): TickLayer[] {
    ...
    if (show16) layers.push({ spacingUnit: 0.25, styleKey: "sub", tickHeight: 4, skipModulo: 4 });
        ...
        layers.push({ spacingUnit: 0.5, styleKey: "sub", tickHeight: 5, skipModulo: 2 });
    if (showBt) {
        layers.push({
            spacingUnit: 1,
            styleKey: "beat",
    ...
    if (subBarGroup > 0)
        layers.push({
            spacingUnit: subBarGroup * bpb,
            styleKey: "beat",
            tickHeight: 11,
            skipModulo: barGroup / subBarGroup,
        });
    layers.push({
        spacingUnit: barGroup * bpb,
        styleKey: "bar",
        isMajor: true,
        label: (b) => {
            const bar = Math.floor(b / bpb);
            return String(bar >= 0 ? bar + 1 : bar);
        },
        labelStyle: "major",
    });
    return layers;
