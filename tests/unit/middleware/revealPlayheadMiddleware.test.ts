...
function makeStore(): EnhancedStore {
  return configureStore({
    ...
    middleware: (getDefault) =>
      getDefault().prepend(revealPlayheadMiddleware.middleware),
  })
}
...
describe('revealPlayheadMiddleware', () => {
  ...
  beforeEach(() => {
    ...
    store.dispatch(setVideo({
      path: '/v.mp4', originalName: 'v.mp4', videoUrl: '',
      duration: 100, fps: 30, fileHash: 'h',
    }))
  ...
  it('is a no-op before a video has loaded', () => {
    const bare = configureStore({
      reducer: {
        video: videoReducer, ui: uiReducer, warp: warpReducer,
        region: regionReducer, history: historyReducer, scene: sceneReducer,
        thumbnails: thumbnailsReducer, settings: settingsReducer, lists: listsReducer,
      },
