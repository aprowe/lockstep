...
const target = (time: number, source: SnapSource = "anchor", id?: string | number): SnapTarget => ({
    time,
    source,
    id,
});
...
describe("computeSnap — contract", () => {
    it("returns no snap when subjects is empty", () => {
        expect(computeSnap({ subjects: [], targets: [target(0)], thresholdSec: 1 })).toEqual({
            delta: 0,
            hit: null,
        });
    });
    ...
    it("returns no snap when there are no targets and no grid", () => {
        expect(computeSnap({ subjects: [1], thresholdSec: 1 })).toEqual({ delta: 0, hit: null });
    });

    it("returns no snap when threshold is zero or negative", () => {
        expect(computeSnap({ subjects: [1], targets: [target(1)], thresholdSec: 0 })).toEqual({
            delta: 0,
            hit: null,
        });
        expect(computeSnap({ subjects: [1], targets: [target(1)], thresholdSec: -0.5 })).toEqual({
            delta: 0,
            hit: null,
        });
    });
