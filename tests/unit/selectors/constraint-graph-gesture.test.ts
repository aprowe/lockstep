...
describe("selectConstraintGraph: gesture-scoped constraints visible", () => {
    ...
    it("graph contains profile SnapTarget for pair drag", () => {
        ...
        const tg = graph.constraints.find(
            (c) =>
                c.kind === ConstraintKind.TranslateGroup &&
                (c as { tag?: string }).tag === "gesture:pair:1",
        );
    ...
    it("snap hints: graph from selector → findSnapCandidates returns candidates within radius", async () => {
        ...
        const populated = graph.constraints.find(
            (c) =>
                c.kind === ConstraintKind.SnapTarget &&
                (c as { id: string }).id === "r-in" &&
                (c as { targets: { entityId: string }[] }).targets.length > 0,
        );
        expect(
            populated,
            "SnapTarget with populated targets is in the selector graph",
        ).toBeDefined();
        ...
        expect(findSnapCandidates(graph, "r-in", Field.In, 10, 20), "outside radius").toHaveLength(
            0,
        );
        expect(
            findSnapCandidates(graph, "r-in", Field.In, 8, 18).length,
            "inside radius (in=8, d=3)",
        ).toBeGreaterThan(0);
        expect(
            findSnapCandidates(graph, "r-in", Field.In, 6, 16).length,
            "inside radius (in=6, d=1)",
        ).toBeGreaterThan(0);
        expect(
            findSnapCandidates(graph, "r-in", Field.In, 5, 15)[0].distance,
            "on target d=0",
        ).toBeCloseTo(0, 6);
    });
});
