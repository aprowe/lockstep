...
describe('ConformRedirect: clipout edge drag while conformed', () => {
  it('drag clipout.in by +0.5: clipout AND anchor.beat both move by 0.5; orig unchanged', () => {
    ...
    expect(s.warp.origAnchors.find(a => a.id === 1)!.time, 'orig unchanged')
      .toBeCloseTo(10, 6)
    expect(s.warp.beatAnchors.find(a => a.id === 1)!.time, 'anchor.beat absorbed the redirect')
      .toBeCloseTo(10.5, 6)
  })
  ...
  it('lasso clipin + clipout (no anchor), drag clipin while conformed: clipout = anchor.beat (override)', () => {
    ...
    store.dispatch({ type: 'lists/setListSelection', payload: { list: 'clipout', ids: ['r'] } })
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 1)!.time, 'anchor.beat untouched')
      .toBeCloseTo(10, 6)
  })
})
