...
function setupAnchorPairs(...times: number[]): State {
    ...
    state = reduce(state, {
        ...
        constraint: { kind: ConstraintKind.SnapCohort, tag: "anchor-in", ids: inIds },
    });
    state = reduce(state, {
        kind: OpKind.AddConstraint,
        constraint: { kind: ConstraintKind.SnapCohort, tag: "anchor-out", ids: outIds },
    });
    // anchor-in drags snap to anchor-out targets (per SNAP_RULES table).
    state = reduce(state, {
        kind: OpKind.AddConstraint,
        constraint: { kind: ConstraintKind.SnapRule, dragger: "anchor-in", target: "anchor-out" },
    });
    return state;
}
