...
describe('Profile anchor drag of diverged anchor onto clipin: conform engages on release', () => {
  it('drag orig from 10 to 15 (lands on clipin.in): clipout diverges to [5, 20]', () => {
    ...
    const region: Region = {
      id: 'r', name: 'r', inPoint: 15, outPoint: 20,
      inBeatTime: 15, outBeatTime: 20, defaultLinked: true,
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 1)!.time, 'beat at 5 (unlinked, unchanged)').toBeCloseTo(5, 6)
