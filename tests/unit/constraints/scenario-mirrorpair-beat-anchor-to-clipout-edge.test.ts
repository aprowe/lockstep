...
describe('MirrorPair: beat-anchor drag ↔ clipout edge coupling', () => {
  ...
  it('LINKED pair, full conform: beat-anchor drag pulls coincident clipout edge', () => {
    ...
    const region: Region = {
      id: 'r', name: 'r', inPoint: 5, outPoint: 25,
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 1)?.time, 'beat moved to 7').toBeCloseTo(7, 6)
    expect(s.region.regions[0].inBeatTime, 'clipout.in followed beat via MirrorPair').toBeCloseTo(7, 6)
    expect(s.region.regions[0].outBeatTime, 'clipout.out unchanged (not coincident with this anchor)').toBeCloseTo(25, 6)
  })
  ...
  it('DIVERGED pair (only beat coincides): solo beat-anchor drag does NOT pull clipout edge', () => {
    ...
    const store = makeStore()
    const region: Region = {
      id: 'r', name: 'r', inPoint: 7, outPoint: 25,
      inBeatTime: 10, outBeatTime: 25, defaultLinked: false,
      bpm: 120, lockedBeats: 20,
      minStretch: 0.5, maxStretch: 2.0,
    }
    store.dispatch(addRegion(region))
    ...
    expect(s.region.regions[0].inBeatTime, 'clipout.in MUST stay put — guard rejects').toBeCloseTo(10, 6)
