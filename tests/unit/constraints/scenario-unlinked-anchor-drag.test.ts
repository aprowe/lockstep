...
describe('Explicitly unlinked anchor: orig drag does not pull beat', () => {
  ...
  it('after moveBeatAnchor (programmatic unlink), dragging orig keeps beat in place', () => {
    ...
    expect(s.warp.origAnchors.find(a => a.id === 1)?.time, 'orig moved to 8').toBeCloseTo(8, 6)
    expect(s.warp.beatAnchors.find(a => a.id === 1)?.time, 'beat must stay at 10').toBeCloseTo(10, 6)
  })
  ...
  it('after beat-anchor DRAG diverges the pair, dragging orig keeps beat in place', () => {
    ...
    expect(beatAfterBeatDrag?.linked, 'pair must be marked unlinked after beat drag').toBe(false)
