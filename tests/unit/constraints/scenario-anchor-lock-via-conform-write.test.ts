...
describe('Anchor-lock follows clipout writes regardless of source', () => {
  it('lockMode=bpm: dragging A1.beat translates inner A2 via the lock TranslateGroup (driver accepts tagged write)', () => {
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 1)!.time, 'A1.beat at 12').toBeCloseTo(12, 6)
    expect(s.warp.beatAnchors.find(a => a.id === 3)!.time, 'A3.beat at 32').toBeCloseTo(32, 6)
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 2)!.time, 'A2 translated by +2 via lock')
      .toBeCloseTo(22, 6)
  })
  ...
  it('symmetric: dragging clipout.in directly OR dragging A1.beat both rescale A2', () => {
    ...
    expect(after1.warp.beatAnchors.find(a => a.id === 2)!.time, 'path 1 — A2 after clipout drag')
      .toBeCloseTo(21, 6)
    expect(after2.warp.beatAnchors.find(a => a.id === 2)!.time, 'path 2 — A2 after anchor.beat drag')
      .toBeCloseTo(21, 6)
    expect(after1.warp.beatAnchors.find(a => a.id === 2)!.time)
      .toBeCloseTo(after2.warp.beatAnchors.find(a => a.id === 2)!.time, 6)
  })
  ...
  it('dragging anchor.out (the beat side of edge-conformed A1) carries the inner anchor', () => {
    ...
    expect(store.getState().warp.beatAnchors.find(a => a.id === 2)!.time)
      .toBeCloseTo(20, 6)
    ...
    const s = store.getState()
    const r = s.region.regions[0]
    expect(s.warp.beatAnchors.find(a => a.id === 1)!.time, 'A1.beat moved to 12')
      .toBeCloseTo(12, 6)
    ...
    expect(s.warp.beatAnchors.find(a => a.id === 2)!.time, 'A2 rescaled via lock ScaleGroup')
      .toBeCloseTo(21, 6)
  })
})
