...
describe('runConstraintPipeline — equivalence with applyOp path', () => {
  ...
  it('1. orig anchor Move — single anchor, no selection', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.Move, id: anchorInId(1), delta: 2.0 } })
  ...
  it('2. beat anchor Move — single beat anchor', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.Move, id: anchorOutId(1), delta: 1.5 } })
    const newState = applyDiffsToSlice(slice, output)
  ...
  it('6. clipout body pan — Move on clipout entity', () => {
    ...
    slice = withRegion(slice, makeRegion({
      id: 'r1', inPoint: 0, outPoint: 10,
      inBeatTime: 2, outBeatTime: 12,
      defaultLinked: false,
    }))
    const dragCtx = emptyDragCtx()

    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.Move, id: regionOutId('r1'), delta: 3.0 } })
    const newState = applyDiffsToSlice(slice, output)
  ...
  it('7. clipout edge resize — SetEdge on out edge', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.SetEdge, id: regionOutId('r1'), edge: 'out', value: 15.0 } })
    const newState = applyDiffsToSlice(slice, output)
  ...
  it('8. clipin edge resize — SetEdge on in edge', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.SetEdge, id: regionInId('r1'), edge: 'in', value: 4.0 } })
    const newState = applyDiffsToSlice(slice, output)
  ...
  it('10. bpmDerivedConstraint — clipout SetEdge updates lockedBeats (fixed=bpm)', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.SetEdge, id: regionOutId('r1'), edge: 'out', value: 20.0 } })
    const newState = applyDiffsToSlice(slice, output)
  ...
  it('13. SetValue bpm — updates meta, no entity position writes', () => {
    ...
    const output = runConstraintPipeline({ slice, dragCtx, op: { kind: OpKind.SetValue, id: regionOutId('r1'), field: 'bpm', value: 140 } })
