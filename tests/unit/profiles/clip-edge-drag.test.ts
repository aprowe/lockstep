...
const ctx: ProfileContext = {
  preDrag: {
    ...
    regions: [{
      ...
      inBeatTime: 15, outBeatTime: 25, defaultLinked: false,
    }],
  },
...
describe('CLIP_EDGE_DRAG profile', () => {
  it('onDrag clipin in-edge: SetEdge with preDrag inPoint + delta', () => {
    const ops = CLIP_EDGE_DRAG.onDrag({ kind: 'clip-in-edge', clipId: 'r1', space: 'input' }, 3, ctx)
  ...
  it('onDrag clipin out-edge: SetEdge with preDrag outPoint + delta', () => {
    const ops = CLIP_EDGE_DRAG.onDrag({ kind: 'clip-out-edge', clipId: 'r1', space: 'input' }, 5, ctx)
    expect(ops).toHaveLength(1)
  ...
  it('onDrag clipout in-edge: SetEdge with preDrag inBeatTime + delta', () => {
    const ops = CLIP_EDGE_DRAG.onDrag({ kind: 'clip-in-edge', clipId: 'r1', space: 'beat' }, 2, ctx)
    expect(ops).toHaveLength(1)
