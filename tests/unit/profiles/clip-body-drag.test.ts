...
describe("CLIP_BODY_DRAG profile", () => {
    it("onDrag (input space) emits Move op on the clipin entity", () => {
        const ops = CLIP_BODY_DRAG.onDrag(
            { kind: "clip-body", clipId: "r1", space: "input" },
            3,
            ctx,
        );
    ...
    it("onDrag (beat space) emits Move op on the clipout entity", () => {
        const ops = CLIP_BODY_DRAG.onDrag(
            { kind: "clip-body", clipId: "r1", space: "beat" },
            3,
            ctx,
        );
        expect(ops).toHaveLength(1);
    ...
    it("whileDragging installs a body-mode SnapTarget on the dragged clip", () => {
        const cs = CLIP_BODY_DRAG.whileDragging(
            { kind: "clip-body", clipId: "r1", space: "input" },
            ctx,
            state,
        );
        const st = cs.find((c) => c.kind === ConstraintKind.SnapTarget) as
            | { kind: string; id: string; mode: string }
            | undefined;
