...
import type { Snapshot, PointerEventLike, HitEntry, Intent } from "../../../src/timeline/types";
...
describe("controller.pointerDown", () => {
    ...
    it("clicking on an anchor (input) sets anchor drag and defers anchorSelect to pointerUp", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 100, clientY: 200 }), snap);
    ...
    it("clicking on a region-edge sets region-edge drag and defers regionSelect to pointerUp", () => {
        ...
        const snap = makeSnapshot({
            ...
            hits: [
                pointHit(100, 200, { kind: "region-edge", id: "r1", edge: "out", isOutput: false }),
            ],
        });
        const intents = c.pointerDown(makePointerEvent({ clientX: 100, clientY: 200 }), snap);
        ...
        if (ds?.kind === "region-edge") {
            ...
            expect(ds.profileHandle).toEqual({
                kind: "clip-out-edge",
                clipId: "r1",
                space: "input",
            });
        ...
        const up = c.pointerUp(snap);
        expect(up.find((i) => i.kind === "regionSelect")).toEqual({
            kind: "regionSelect",
            id: "r1",
        });
    });
    ...
    it("clicking on a region body sets region-move drag and defers regionSelect to pointerUp", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 120, clientY: 200 }), snap);
    ...
    it("region-edge on output uses regionsOutput when present", () => {
        ...
        const snap = makeSnapshot({
            ...
            hits: [
                pointHit(100, 200, { kind: "region-edge", id: "r3", edge: "in", isOutput: true }),
            ],
        });
    ...
    it("click on the time ruler emits seek and sets seek drag (space=input)", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 400, clientY: cy }), snap);
        expect(intents.length).toBe(1);
    ...
    it("click on the time ruler past duration is clamped to MAX", () => {
        ...
        const timeTrack = snap.tracks.find((t) => t.id === "time")!;
        const cy = timeTrack.y + timeTrack.h / 2;
        const intents = c.pointerDown(makePointerEvent({ clientX: 700, clientY: cy }), snap);
        const seek = intents[0];
        expect(seek.kind).toBe("seek");
    ...
    it("click on the beat ruler emits seekBeat (space=output)", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 200, clientY: cy }), snap);
        expect(intents.length).toBe(1);
        const seek = intents[0];
    ...
    it("click in an empty area arms a non-active lasso with empty initial sets", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 300, clientY: cy }), snap);
        expect(intents).toEqual([]);
        const ds = c.getDragState();
    ...
    it("clicking a SELECTED anchor with regions selected leaves selection intact (lasso TG handles regions)", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 80, clientY: 200 }), snap);
    ...
    it("clicking an UNSELECTED anchor (no modifier) defers replace-select to pointerUp and captures only that id", () => {
        ...
        const intents = c.pointerDown(makePointerEvent({ clientX: 400, clientY: 200 }), snap);
...
describe("controller.pointerUp", () => {
    ...
    it("region-edge drag (clipin): emits drag lifecycle (profile path)", () => {
        ...
        const snap = makeSnapshot({
            regions: [{ id: "r1", inPoint: 10, outPoint: 30 }],
            hits: [
                pointHit(240, 200, { kind: "region-edge", id: "r1", edge: "out", isOutput: false }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 240, clientY: 200 }), snap);
        c.pointerMove(makePointerEvent({ clientX: 400, clientY: 200 }), snap);
        const intents = c.pointerUp(snap);
        const dragFinal = intents.find((i) => i.kind === "drag") as
            | Extract<Intent, { kind: "drag" }>
            | undefined;
    ...
    it("seek/pan/minimap drags: no commit intents, just cleanup", () => {
        // seek
        {
            ...
            const commits = intents.filter(
                (i) =>
                    i.kind === "anchorsChanged" ||
                    i.kind === "beatAnchorsChanged" ||
                    i.kind === "regionResize" ||
                    i.kind === "regionMove" ||
                    i.kind === "clipsSelectionChange" ||
                    i.kind === "scenesSelectionChange" ||
                    i.kind === "connectorSelectionChange" ||
                    i.kind === "seek" ||
...
describe("controller.doubleClick", () => {
    it("on input anchor: emits anchorDelete", () => {
        ...
        expect(c.doubleClick(makePointerEvent({ clientX: 100, clientY: 200 }), snap)).toEqual([
            { kind: "anchorDelete", id: 5 },
        ]);
    });
    ...
    it("on scene: emits sceneDelete", () => {
        ...
        expect(c.doubleClick(makePointerEvent({ clientX: 200, clientY: 50 }), snap)).toEqual([
            { kind: "sceneDelete", time: 22 },
        ]);
    });
...
describe("controller.contextMenu", () => {
    it("on input anchor: emits anchorContextMenu with client coords", () => {
        ...
        const intents = c.contextMenu(makePointerEvent({ clientX: 100, clientY: 200 }), snap);
    ...
    it("on scene: emits sceneContextMenu", () => {
        ...
        const intents = c.contextMenu(makePointerEvent({ clientX: 200, clientY: 50 }), snap);
    ...
    it("on empty area: emits timelineContextMenu with time", () => {
        ...
        const intents = c.contextMenu(makePointerEvent({ clientX: 400, clientY: 200 }), snap);
        expect(intents.length).toBe(1);
...
describe("controller.keyDown", () => {
    ...
    it("Cmd+D → timelineDeselect", () => {
        const c = createTimelineController();
        expect(c.keyDown(keyEvent({ key: "d", metaKey: true }))).toEqual([
    ...
    it("Ctrl+D → timelineDeselect", () => {
        const c = createTimelineController();
        expect(c.keyDown(keyEvent({ key: "D", ctrlKey: true }))).toEqual([
            { kind: "timelineDeselect" },
        ]);
    });
...
describe("controller.pointerMove — pan drag", () => {
    it("emits viewChange shifted by pixel delta", () => {
        ...
        const intents = c.pointerMove(
            makePointerEvent({ clientX: 280, clientY: 200, altKey: true }),
            snap,
        );
...
describe("controller.pointerMove — minimap drag", () => {
    it("emits viewChange recentered around cursor x", () => {
        ...
        c.pointerDown(makePointerEvent({ clientX: 200, clientY: MINIMAP_H / 2 }), snap);
        const intents = c.pointerMove(
            makePointerEvent({ clientX: 600, clientY: MINIMAP_H / 2 }),
            snap,
        );
        const vc = intents.find((i) => i.kind === "viewChange")!;
...
describe("controller.pointerMove — region-edge drag", () => {
    ...
    it("output region-edge emits drag intent (profile path) with isOutput=true space", () => {
        ...
        const snap = makeSnapshot({
            ...
            hits: [
                pointHit(240, 200, { kind: "region-edge", id: "r1", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 240, clientY: 200 }), snap);
        const intents = c.pointerMove(makePointerEvent({ clientX: 400, clientY: 200 }), snap);
...
describe("controller — clipout (isOutput=true) region drag", () => {
    ...
    it("clicking a clipout region EDGE (isOutput=true) arms a region-edge drag", () => {
        ...
        const snap = makeSnapshot({
            ...
            hits: [
                pointHit(150, 200, { kind: "region-edge", id: "re", edge: "out", isOutput: true }),
            ],
        });
    ...
    it("clipout edge drag: profile path emits drag with edge delta", () => {
        ...
        const snap = makeSnapshot({
            ...
            regionsOutput: [{ id: "re", inPoint: 1.0, outPoint: 4.0 }],
            hits: [
                pointHit(320, 200, { kind: "region-edge", id: "re", edge: "out", isOutput: true }),
            ],
        });
    ...
    it("clipout drag does NOT emit input-space region commit (clipin stays put)", () => {
        ...
        const inputMoves = intents.filter(
            (i) => (i.kind === "regionMove" || i.kind === "regionResize") && !i.isOutput,
        );
...
describe("controller — R4 (new): clipout edge drag does NOT carry linked beat anchor in controller", () => {
    ...
    function makeR4Snap() {
        return makeSnapshot({
            ...
            beatAnchors: [{ id: 5, time: 20 }],
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "r1", edge: "out", isOutput: true }),
            ],
        });
    }
    ...
    it("input-space (non-output) edge drag does NOT have liveBeatAnchors", () => {
        ...
        const snap = makeSnapshot({
            ...
            anchors: [],
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "r1", edge: "out", isOutput: false }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        const ds = c.getDragState();
...
describe("controller — Slice A: live drag intent emitted during clipout edge drag", () => {
    ...
    it("lock=bpm: pointerMove emits drag intent with delta=5", () => {
        ...
        const snap = makeSnapshot({
            ...
            clipLock: "bpm",
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "rA", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
    ...
    it("lock=beats: pointerMove emits drag intent with delta=5", () => {
        ...
        const snap = makeSnapshot({
            ...
            clipLockedBeats: 40,
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "rB", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        // Drag out-edge from t=20 to t=25 (x=200) — delta = 5
    ...
    it("no clipLock in snapshot → drag intent still emitted (no lock-dependent derivation)", () => {
        ...
        const snap = makeSnapshot({
            ...
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "rC", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        const intents = c.pointerMove(makePointerEvent({ clientX: 200, clientY: 200 }), snap);
    ...
    it("input-space (non-output) edge drag does NOT emit output regionResize", () => {
        ...
        const snap = makeSnapshot({
            ...
            clipLock: "bpm",
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "rD", edge: "out", isOutput: false }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        const intents = c.pointerMove(makePointerEvent({ clientX: 200, clientY: 200 }), snap);
    ...
    it("pointerUp clears via pubClearGesture (no separate clear intents needed)", () => {
        ...
        const snap = makeSnapshot({
            ...
            clipLock: "bpm",
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "rE", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        c.pointerMove(makePointerEvent({ clientX: 200, clientY: 200 }), snap);
...
describe("controller — Slice B: live beat-anchor rescale during clipout edge drag", () => {
    ...
    function makeSliceBEdgeSnap(clipAnchorLock: boolean, clipLock: "bpm" | "beats") {
        return makeSnapshot({
            ...
            clipAnchorLock,
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "sB", edge: "out", isOutput: true }),
            ],
        });
    }
    ...
    it("anchor outside [origIn, origOut] is NOT rescaled", () => {
        ...
        const snap = makeSnapshot({
            ...
            clipAnchorLock: true,
            hits: [
                pointHit(160, 200, { kind: "region-edge", id: "sBO", edge: "out", isOutput: true }),
            ],
        });
        c.pointerDown(makePointerEvent({ clientX: 160, clientY: 200 }), snap);
        const intents = c.pointerMove(makePointerEvent({ clientX: 200, clientY: 200 }), snap);
