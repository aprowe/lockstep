...
export function buildScene(n: number, seed: number): Scene {
    ...
    const anchorTimes = Array.from({ length: n }, () => 10 + rng() * span).sort((a, b) => a - b);
    ...
    const regionWidth = Math.max(2, span / Math.max(4, n));
    const regionCenters = Array.from({ length: n }, () => 10 + rng() * span).sort(
        (a, b) => a - b,
    );
    ...
    const sceneTimes = Array.from({ length: n }, () => rng() * DURATION).sort((a, b) => a - b);
