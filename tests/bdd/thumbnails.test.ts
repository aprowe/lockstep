...
function renderWithStore(store: ReturnType<typeof makeStore>, ui: ReactElement) {
    return render(
        createElement(Provider, {
            store,
            children: createElement(ThumbnailHoverProvider, null, ui),
        }),
    );
}
...
describeFeature(feature, ({ Scenario, AfterEachScenario }) => {
        ...
        Then("thumbnail generation starts in the background", () => {
            expect(vi.mocked(invoke)).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    req: expect.objectContaining({ file_hash: "abc", fps: 30 }),
                }),
            );
        });
    });
    ...
    Scenario("Thumbnails near the playhead are generated first", ({ Given, When, Then }) => {
        ...
        Then("thumbnails near the playhead are generated before thumbnails elsewhere", () => {
            ...
            const call = vi
                .mocked(invoke)
                .mock.calls.find((c) => c[0] === "set_thumbnail_priority")!;
    ...
    Scenario(
        "Scrubbing the [input ruler] updates the thumbnail viewer",
        ({ Given, When, Then }) => {
            ...
            Then(
                "the thumbnail viewer shows the playhead frame plus as many surrounding frames as fit in the viewer",
                () => {
    ...
    Scenario(
        "Filmstrip center slot equals the toolbar frame counter",
        ({ Given, When, Then, And }) => {
            ...
            When(
                "the playhead is at a time whose frame conversion is ambiguous due to floating-point error",
                () => {
            ...
            Then(
                "the center slot of the filmstrip shows exactly the frame displayed by the toolbar frame counter",
                () => {
    ...
    Scenario("Missing thumbnails show a placeholder", ({ Given, When, Then }) => {
        ...
        When(
            "a thumbnail is requested for a frame whose thumbnail has not been generated yet",
            () => {
    ...
    Scenario(
        "Expanded scene strip shows one thumbnail per marker",
        ({ Given, And, When, Then }) => {
            const store = makeStore();
            let container: HTMLElement | null = null;
            Given("[a video is loaded]", () => {
            ...
            Then(
                "a thumbnail of each scene marker's frame is shown inline inside the scene strip",
                () => {
