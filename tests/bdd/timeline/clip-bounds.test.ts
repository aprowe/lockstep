...
import {
  moveRegionBounds,
  panClipinBounds,
} from '../../../src/store/thunks/regionThunks'
import {
...
import {
  commitClipoutResize,
  commitClipoutPan,
} from '../../../src/store/thunks/clipoutThunks'
...
describeFeature(feature, ({ Scenario, ScenarioOutline, Rule }) => {
  ...
  Scenario('Setting either beat-space bound away from its input partner diverges the clip', ({ Given, When, Then, And }) => {
    let store: Store
    Given('a default-linked clip from {number} to {number}', (_ctx, a: number, b: number) => {
      store = setupClip({ inPoint: a, outPoint: b })
    })
    When('inBeatTime or outBeatTime is set to a value not matching its input partner', () => {
      store.dispatch(commitClipoutResize({ id: 'r', inBeatTime: 5, outBeatTime: 20, altKey: false }))
    })
  ...
  ScenarioOutline('A clipin <edge>-edge edit is undoable', ({ Given, When, And, Then }, variables) => {
    let store: Store
    let originalInPoint = 0, originalOutPoint = 0
    Given('a clip from {number} to {number}', (_ctx, a: number, b: number) => {
      store = setupClip({ inPoint: a, outPoint: b })
      originalInPoint = a; originalOutPoint = b
    ...
    When('the <edge>-point is changed to <new>', () => {
      ...
      store.dispatch(moveRegionBounds({
        id: 'r',
  ...
  Scenario('Setting in-point past out-point shifts the clip to preserve length', ({ Given, When, Then }) => {
    let store: Store
    Given('a clip from {number} to {number}', (_ctx, a: number, b: number) => {
      store = setupClip({ inPoint: a, outPoint: b })
    })
    When('the in-point is changed to {number}', (_ctx, newIn: number) => {
      store.dispatch(moveRegionBounds({ id: 'r', inPoint: newIn, outPoint: clip(store).outPoint }))
    })
  ...
  ScenarioOutline('Set-<Edge>-Point outside the clip creates a new clip', ({ Given, When, Then }) => {
  ...
  ScenarioOutline('A clip cannot resize below the minimum length', ({ Given, When, Then }, variables) => {
    let store: Store
    Given('a clip from {number} to {number}', (_ctx, a: number, b: number) => {
      store = setupClip({ inPoint: a, outPoint: b })
    })
    When('the clip is resized to span <a> to <b>', () => {
      store.dispatch(moveRegionBounds({ id: 'r', inPoint: Number(variables.a), outPoint: Number(variables.b) }))
    })
  ...
  Rule('Object isolation — anchors and clip edges move independently', ({ RuleBackground, RuleScenario }) => {
    let store: Store
    ...
    RuleScenario("Dragging an anchor doesn't move a clip edge", ({ Given, When, Then }) => {
      ...
      Then('inPoint stays at {number} and only the anchor moves', (_ctx, inPoint: number) => {
        expect(clip(store).inPoint).toBe(inPoint)
        expect(origAt(store, 1)).toBe(17)
      })
    })

    RuleScenario("Dragging a clip edge doesn't move an anchor", ({ Given, When, Then }) => {
      Given('an anchor at {number}', (_ctx, t: number) => {
        addAnchorPair(store, 1, t)
      })
      When('the user drags the clipin in-edge', () => {
        store.dispatch(moveRegionBounds({ id: 'r', inPoint: 13, outPoint: clip(store).outPoint }))
      })
      Then('the anchor stays at {number} and only the edge moves', (_ctx, t: number) => {
        expect(origAt(store, 1)).toBe(t)
        expect(clip(store).inPoint).toBe(13)
      })
    })

    RuleScenario('Dragging a default-linked clipin moves its clipout too', ({ Given, When, Then }) => {
      Given('the clip is default-linked', () => {
        expect(clip(store).defaultLinked).toBe(true)
      })
      When('the user drags the clipin so the clip spans {number} to {number}', (_ctx, a: number, b: number) => {
        store.dispatch(panClipinBounds({ id: 'r', inPoint: a, outPoint: b }))
      })
      Then('the clipout spans {number} to {number}', (_ctx, a: number, b: number) => {
        const r = clip(store)
  ...
  Rule('Conform — clipout edge tracks the paired beat anchor\'s beat time', ({ RuleBackground, RuleScenario, RuleScenarioOutline }) => {
    let store: Store
    RuleBackground(({ Given }) => {
      Given('a default-linked clip from {number} to {number}', (_ctx, a: number, b: number) => {
        store = setupClip({ inPoint: a, outPoint: b })
    ...
    RuleScenarioOutline('A linked anchor pair conformed at <edge> displays the clipout edge at the paired beat time', ({ Given, Then }, variables) => {
      Given('an anchor pair at orig <edgeVal>, beat <beatVal>', () => {
        addAnchorPair(store, 1, Number(variables.edgeVal), Number(variables.beatVal))
      })
      Then('the clipout <edge>-edge displays at <beatVal>', () => {
        ...
        if (!entity || entity.kind !== 'clip') throw new Error('clipout entity missing')
        const edge = variables.edge as 'in' | 'out'
    ...
    RuleScenario('A clipin drag past a linked anchor temporarily conforms', ({ Given, When, Then, And }) => {
      Given('an anchor pair at orig {number}, beat {number}', (_ctx, orig: number, beat: number) => {
        addAnchorPair(store, 1, orig, beat)
      })
      When('the user drags the clipin in-edge from {number} through {number} and out to {number}', (_ctx, _start: number, mid: number, end: number) => {
        store.dispatch(moveRegionBounds({ id: 'r', inPoint: mid, outPoint: 20 }))
        expect(clip(store).inBeatTime).toBeCloseTo(mid, 6) // conform holds (orig=beat=mid)
        store.dispatch(moveRegionBounds({ id: 'r', inPoint: end, outPoint: 20 }))
      })
      Then('while the in-edge coincides with {number} the conform holds and clipout in-edge tracks {number}', () => {
        // Asserted mid-sweep above.
      })
      And('as the in-edge passes {number} the conform releases and the edge continues with the cursor', (_ctx, mid: number) => {
        const r = clip(store)
        expect(r.inPoint).toBeGreaterThan(mid)
        expect(r.inBeatTime).toBeCloseTo(r.inPoint, 6)
      })
      And('the anchor pair stays at orig {number}, beat {number} throughout', (_ctx, orig: number, beat: number) => {
        expect(origAt(store, 1)).toBe(orig)
        expect(beatAt(store, 1)).toBe(beat)
      })
    })

    RuleScenario('An orig-anchor drag across a clip edge temporarily conforms', ({ Given, When, Then, And }) => {
      Given('an anchor pair at orig {number}, beat {number}', (_ctx, orig: number, beat: number) => {
        addAnchorPair(store, 1, orig, beat)
      })
      When('the user drags the orig anchor from {number} through {number} and out to {number}', (_ctx, _start: number, _mid: number, end: number) => {
      ...
      And('dragging orig past {number} releases the conform', (_ctx, edgeVal: number) => {
        expect(origAt(store, 1)).toBeGreaterThan(edgeVal)
      })
      And('the clip\'s inPoint stays at {number} throughout', (_ctx, inPoint: number) => {
        expect(clip(store).inPoint).toBe(inPoint)
      })
    })

    RuleScenario('Clipin body drag onto a diverged anchor writes clipout to the paired beat time', ({ Given, When, Then, And }) => {
      let preDragIn = 0
      Given('the clip spans {number} to {number} and an anchor pair at orig {number}, beat {number} diverged', (_ctx, a: number, b: number, orig: number, beat: number) => {
      ...
      When('the user drags the clipin body so inPoint sequentially reaches {number}, then {number}, then {number}', () => {
        // Sequential delta dispatches handled below in the Then steps.
      })
      Then('at inPoint {number} pre-anchor inBeatTime is {number} and beat anchor stays at {number}', (_ctx, target: number, expectedBeat: number, beat: number) => {
        store.dispatch(applyRegionEntityMove({ id: 'r', delta: target - preDragIn }))
      ...
      And('at inPoint {number} on-anchor inBeatTime is {number} and beat anchor stays at {number}', (_ctx, target: number, expectedBeat: number, beat: number) => {
        store.dispatch(applyRegionEntityMove({ id: 'r', delta: target - preDragIn }))
        const post = clip(store)
        expect(post.inPoint).toBeCloseTo(target, 6)
        expect(post.inBeatTime).toBeCloseTo(expectedBeat, 6)
      ...
      And('at inPoint {number} past-anchor inBeatTime is {number} and beat anchor stays at {number}', (_ctx, target: number, expectedBeat: number, beat: number) => {
        store.dispatch(applyRegionEntityMove({ id: 'r', delta: target - preDragIn }))
        const post = clip(store)
        expect(post.inPoint).toBeCloseTo(target, 6)
        expect(post.inBeatTime).toBeCloseTo(expectedBeat, 6)
    ...
    RuleScenarioOutline('Edge resize onto a diverged anchor writes only the matching clipout edge', ({ Given, When, Then, And }, variables) => {
      Given('the clip spans {number} to {number} and an anchor pair at orig <origVal>, beat <beatVal> diverged', (_ctx, a: number, b: number) => {
        store.dispatch(deleteRegion('r'))
        store.dispatch(addRegion(makeClip({ inPoint: a, outPoint: b })))
        store.dispatch(setActiveRegionId('r'))
        addAnchorPair(store, 1, Number(variables.origVal), Number(variables.beatVal))
      })
      ...
      Then('the clipout becomes <clipoutResult>', () => {
        const m = String(variables.clipoutResult).match(/\(([^,]+),\s*([^)]+)\)/)!
    ...
    RuleScenario('Clipin drag past a diverged anchor with no output coincidence does NOT pull the beat anchor', ({ Given, When, Then }) => {
      Given('the clip spans {number} to {number} and an anchor pair at orig {number}, beat {number} diverged', (_ctx, a: number, b: number, orig: number, beat: number) => {
        store.dispatch(deleteRegion('r'))
        store.dispatch(addRegion(makeClip({ inPoint: a, outPoint: b })))
        store.dispatch(setActiveRegionId('r'))
      ...
      When('the user drags the clipin body by {number} with the in-edge inside the snap radius of orig', (_ctx, delta: number) => {
    ...
    RuleScenarioOutline('Clipout interaction on a conformed pair carries the anchor and writes the beat-space bound', ({ Given, When, Then, And }, variables) => {
    ...
    RuleScenarioOutline('Clipout edge drag on a fully conformed linked pair (orig=beat) moves clipout and carries the anchor', ({ Given, When, Then, And }, variables) => {
      Given('an anchor pair at orig <val>, beat <val> conformed at the <edge>-edge of clip {number} to {number}', () => {
        addAnchorPair(store, 1, Number(variables.val), Number(variables.val))
      })
  ...
  Rule('Conformed-anchor move — dragging the beat side of a conformed pair moves the clipout edge', ({ RuleScenario, RuleScenarioOutline }) => {

    RuleScenarioOutline('Conformed-anchor drag tracks the clipout edge', ({ Given, When, Then, And }, variables) => {
      let store: Store
    ...
    RuleScenarioOutline('Conformed-anchor move respects clip lock', ({ Given, When, Then, And }, variables) => {
      let store: Store
      Given('a clip with clipout length {number}, BPM {number}, lock=<lock>, lockedBeats {number}', (_ctx, length: number, bpm: number, beats: number) => {
        ...
        store = setupClip({ inPoint: inP, outPoint: outP, bpm, lockedBeats: beats })
    ...
    RuleScenario('Dragging the orig anchor of a conformed pair unconforms the edge', ({ Given, When, Then, And }) => {
      let store: Store
  ...
  Rule('A clipout edge drag rescales the clipout; lock determines what absorbs the length change', ({ RuleBackground, RuleScenario, RuleScenarioOutline }) => {
    let store: Store
    RuleBackground(({ Given }) => {
      Given('a clip with clipout length {number}, BPM {number}, lockedBeats {number}', (_ctx, length: number, bpm: number, beats: number) => {
        store = setupClip({ inPoint: 0, outPoint: length, bpm, lockedBeats: beats })
      })
    })

    RuleScenarioOutline('Clipout edge drag updates beat-time and the lock-dependent value', ({ Given, When, Then, And }, variables) => {
      ...
      When('the user drags the clipout <edge>-edge to make clipout length <newLen>', () => {
        ...
        store.dispatch(commitClipoutResize({ id: 'r', inBeatTime: inB, outBeatTime: outB, altKey: false }))
      })
    ...
    RuleScenarioOutline('Clipout edge drag carries its conformed anchor (inseparable while conformed)', ({ Given, When, Then, And }, variables) => {
    ...
    RuleScenario('Clipout edge drag snaps in output space only', ({ When, Then, And }) => {
    ...
    RuleScenarioOutline('Clipout edge clamps', ({ When, Then }, variables) => {
      When('the user drags an edge such that <violation>', () => {
        if (String(variables.violation).includes('less than 0.1')) {
          store.dispatch(commitClipoutResize({ id: 'r', inBeatTime: 5, outBeatTime: 5, altKey: false }))
        }
      })
      Then('the moving edge clamps to <limit>', () => {
        if (String(variables.violation).includes('less than 0.1')) {
          expect(clip(store).outBeatTime - clip(store).inBeatTime).toBeGreaterThan(0)
        }
      })
    })
  })
  ...
  Rule('A clipout body drag translates both edges by the same delta', ({ RuleBackground, RuleScenario }) => {
    let store: Store
    RuleBackground(({ Given }) => {
      Given('a clip with inBeatTime {number}, outBeatTime {number}, BPM {number}, lockedBeats {number}', (_ctx, inB: number, outB: number, bpm: number, beats: number) => {
        store = setupClip({
          inPoint: inB, outPoint: outB,
          inBeatTime: inB, outBeatTime: outB,
          bpm, lockedBeats: beats,
        })
      })
    })

    RuleScenario('Clipout body drag translates both edges by the drag delta', ({ When, Then, And }) => {
      When('the user drags the clipout body by {number}', (_ctx, delta: number) => {
        store.dispatch(commitClipoutPan({ id: 'r', delta, altKey: false }))
      })
      Then('inBeatTime is {number} and outBeatTime is {number}', (_ctx, a: number, b: number) => {
        const r = clip(store)
    ...
    RuleScenario('Clipout body drag carries any conformed anchors on either edge', ({ Given, When, Then, And }) => {
  ...
  Rule('Changing lock fixes the new quantity; clipout length is untouched', ({ RuleBackground, RuleScenario, RuleScenarioOutline }) => {
    let store: Store
    RuleBackground(({ Given }) => {
      Given('a clip with BPM {number}, lockedBeats {number}, clipout length {number}', (_ctx, bpm: number, beats: number, length: number) => {
        store = setupClip({ inPoint: 0, outPoint: length, bpm, lockedBeats: beats })
      })
    })

    RuleScenarioOutline('Changing lock fixes the new quantity; length is untouched', ({ Given, When, Then, And }, variables) => {
  ...
  Rule('Direct BPM / beats edits use the grid model; the stretch modifier rescales length', ({ RuleScenario, RuleScenarioOutline }) => {

    RuleScenario('Direct BPM edit uses the grid model — length stays', ({ Given, When, Then, And }) => {
      let store: Store
      Given('a clip with BPM {number}, lockedBeats {number}, clipout length {number}', (_ctx, bpm: number, beats: number, length: number) => {
        store = setupClip({ inPoint: 0, outPoint: length, bpm, lockedBeats: beats })
      })
      When('applyBpmEdit is dispatched with newBpm {number}, stretch false', (_ctx, newBpm: number) => {
        store.dispatch(applyBpmEdit({ id: 'r', newBpm, stretch: false }))
      })
      Then('BPM is {number} and lockedBeats is {number}', (_ctx, bpm: number, beats: number) => {
        const r = clip(store)
    ...
    RuleScenario('Direct beats edit on a diverged clip changes length only on the clipout', ({ Given, When, Then, And }) => {
      let store: Store
      Given('a diverged clip with BPM {number}, lockedBeats {number}, inBeatTime {number}, outBeatTime {number}', (_ctx, bpm: number, beats: number, inB: number, outB: number) => {
        store = setupClip({
          inPoint: 10, outPoint: 20,
          inBeatTime: inB, outBeatTime: outB,
          bpm, lockedBeats: beats, defaultLinked: false,
        })
      })
      When('applyBeatsEdit is dispatched with newLockedBeats {number}', (_ctx, newBeats: number) => {
        store.dispatch(applyBeatsEdit({ id: 'r', newLockedBeats: newBeats, stretch: false }))
      })
      Then('lockedBeats is {number} and BPM stays at {number}', (_ctx, beats: number, bpm: number) => {
        const r = clip(store)
        expect(r.lockedBeats).toBeCloseTo(beats, 3)
        expect(r.bpm).toBeCloseTo(bpm, 3)
      })
    ...
    RuleScenarioOutline('Stretch-mode edit on a diverged clip rescales only the clipout', ({ Given, When, Then, And }, variables) => {
      let store: Store
      Given('a diverged clip with inPoint {number}, outPoint {number}, inBeatTime {number}, outBeatTime {number}, BPM {number}, lockedBeats {number}',
        (_ctx, ip: number, op: number, ib: number, ob: number, bpm: number, beats: number) => {
          store = setupClip({
            inPoint: ip, outPoint: op,
            inBeatTime: ib, outBeatTime: ob,
            bpm, lockedBeats: beats, defaultLinked: false,
          })
        })
      When('<edit> is dispatched with stretch true', () => {
        ...
        if (edit.includes('Bpm')) {
          ...
          store.dispatch(applyBpmEdit({ id: 'r', newBpm: bpm, stretch: true }))
        } else {
          const beats = Number(edit.match(/newLockedBeats (\d+)/)![1])
          store.dispatch(applyBeatsEdit({ id: 'r', newLockedBeats: beats, stretch: true }))
        }
      })
      Then('<changed> updates, <kept> stays, and clipout length rescales to {number}', (_ctx, newLen: number) => {
        expect(clip(store).outBeatTime - clip(store).inBeatTime).toBeCloseTo(newLen, 3)
      })
      And('inPoint stays at {number} and outPoint stays at {number}', (_ctx, ip: number, op: number) => {
        const r = clip(store)
        expect(r.inPoint).toBe(ip)
        expect(r.outPoint).toBe(op)
      })
      And('inBeatTime stays at {number}; the clip remains diverged', (_ctx, ib: number) => {
  ...
  Rule('Unconforming — coincidence break preserves last written beat-space coord', ({ RuleScenarioOutline }) => {
    RuleScenarioOutline('Unconforming via different triggers', ({ Given, When, Then, And }, variables) => {
      let store: Store
      Given('a clip\'s in-edge is conformed to an anchor pair', () => {
        store = setupClip()
        addAnchorPair(store, 1, 10, 10)
      ...
      When('<action>', () => {
        ...
        if (action.startsWith('the user drags the orig anchor')) {
          ...
          store.dispatch(panClipinBounds({ id: 'r', inPoint: 14, outPoint: 24 }))
        } else if (action.startsWith('the user deletes')) {
          store.dispatch(removeAnchors([1]))
        }
      })
      Then('the in-edge is no longer conformed', () => {
        const anchor = store.getState().warp.origAnchors.find(a => a.id === 1)
  ...
  Rule('Anchor-lock determines whether inner beat anchors follow clipout gestures', ({ RuleScenarioOutline }) => {

    RuleScenarioOutline('Alt held during a clipout gesture inverts anchor-lock for that gesture only', ({ Given, And, When, Then }, variables) => {
      let store: Store
      ...
      When('the user holds Alt during the drag', () => {
        if (variables.gesture === 'resize') {
          store.dispatch(commitClipoutResize({ id: 'r', inBeatTime: 10, outBeatTime: 18, altKey: true }))
        } else {
          store.dispatch(commitClipoutPan({ id: 'r', delta: 5, altKey: true }))
        }
      })
    ...
    RuleScenarioOutline('Clipout out-edge resize × anchor-lock × lock matrix', ({ Given, And, When, Then }, variables) => {
      let store: Store
      ...
      And('a clip with lock=<lock>, BPM {number}, lockedBeats {number}, clipout length {number}', (_ctx, bpm: number, beats: number, length: number) => {
        store.dispatch(setLockMode(variables.lock as 'bpm' | 'beats'))
        store.dispatch(deleteRegion('r'))
        store.dispatch(addRegion(makeClip({ inPoint: 10, outPoint: 10 + length, bpm, lockedBeats: beats })))
        store.dispatch(setActiveRegionId('r'))
      })
      And('beat anchors at {number} and {number} inside the clipout window {number}..{number}', (_ctx, t1: number, t2: number) => {
        addAnchorPair(store, 1, t1, t1)
        addAnchorPair(store, 2, t2, t2)
      })
      When('the user drags the clipout out-edge to make clipout length {number}', (_ctx, len: number) => {
        const r = clip(store)
        store.dispatch(commitClipoutResize({
          id: 'r',
    ...
    RuleScenarioOutline('Clipout body-pan × anchor-lock', ({ Given, And, When, Then }, variables) => {
      let store: Store
      Given('anchor-lock is <anchorLock>', () => {
        store = setupClip()
        store.dispatch(setAnchorLock(variables.anchorLock === 'ON'))
      })
      And('a clip with inBeatTime {number}, outBeatTime {number}', (_ctx, inB: number, outB: number) => {
        store.dispatch(deleteRegion('r'))
        store.dispatch(addRegion(makeClip({ inPoint: inB, outPoint: outB, inBeatTime: inB, outBeatTime: outB })))
        store.dispatch(setActiveRegionId('r'))
      })
      And('beat anchors at {number}, {number}, and {number} inside the clipout window', (_ctx, a: number, b: number, c: number) => {
  ...
  Rule('A drag gesture is atomic — completion is one undo step; cancellation reverts state with no undo entry', ({ RuleScenario, RuleScenarioOutline }) => {
    ...
    RuleScenarioOutline('Cancelling a drag reverts state without an undo entry', ({ Given, When, Then, And }, variables) => {
      let store: Store
      let preIn = 0
      Given('a <gesture> is in progress', () => {
        ...
        if (g.includes('clipout body')) {
          ...
          store.dispatch(commitClipoutResize({ id: 'r', inBeatTime: 12, outBeatTime: 20, altKey: false }))
  ...
  Rule('The BPM tick grid repositions in real time during gestures that change beat-space positions', ({ RuleScenarioOutline }) => {
  ...
  Rule('Reset Boundary returns a diverged clip to default-linked', ({ RuleScenario }) => {
    RuleScenario('Reset Boundary clears inBeatTime and outBeatTime', ({ Given, When, Then, And }) => {
      Given('a diverged clip with inPoint {number}, outPoint {number}, inBeatTime {number}, outBeatTime {number}', () => {})
    ...
    RuleScenario('Reset Boundary is disabled on a default-linked clip', ({ Given, Then }) => {
    ...
    RuleScenario('Reset Boundary is undoable', ({ Given, When, Then }) => {
      ...
      Then('inBeatTime is {number}, outBeatTime is {number}, and the clip is diverged again', () => {})
    })
  })

  Rule('A clipout edge drag with lock=beats restricts the snap target set', ({ RuleScenario }) => {
    RuleScenario('BPM-changing clipout edge drag excludes grid ticks and beat anchors from snaps', ({ Given, And, Then }) => {
    ...
    RuleScenario('BPM-changing clipout edge drag adds the self-clip\'s clipin bounds as snap targets', ({ Given, And, Then }) => {
      Given('a clip with lock=beats, inPoint {number}, outPoint {number}', () => {})
      And('the clipout out-edge is being dragged in beat space', () => {})
      Then('the clip\'s own clipin bounds at {number} and {number} in beat-space coords are offered as snap targets', () => {})
    })
  })

  Rule('Set-<Edge>-Point with playhead inside a clip resizes the matching edge', ({ RuleScenarioOutline }) => {
    RuleScenarioOutline('Set-<Edge>-Point moves the nearest boundary when the playhead is inside', ({ Given, When, Then, And }) => {
      Given('a clip from {number} to {number} and the playhead at {number}', () => {})
  ...
  Rule('PointerDown activates a clip; coincident-boundary hits resolve to the active clip', ({ RuleScenario, RuleScenarioOutline }) => {
    RuleScenarioOutline('PointerDown on a clip\'s <target> activates it before any drag', ({ Given, When, Then }) => {
    ...
    RuleScenario('Active clip\'s edge wins a coincident-boundary hit test', ({ Given, When, Then }) => {
    ...
    RuleScenario('Non-active clip\'s edge wins when the active clip has no edge there', ({ Given, And, When, Then }) => {
