...
export function makeKey(
    key: string,
    mods: Partial<Pick<KeyEventLike, "shiftKey" | "ctrlKey" | "metaKey" | "altKey">> = {},
): KeyEventLike {
    return {
        key,
...
export function findIntent<K extends Intent["kind"]>(
    intents: Intent[],
    kind: K,
): Extract<Intent, { kind: K }> | undefined {
...
export function regionHit(
    ...
    if (edge === "in")
        return {
            ...
            w: 8,
            h: tr.h,
            data: { kind: "region-edge", id: regionId, edge: "in", isOutput: false },
        };
    if (edge === "out")
        return {
            x: x2 - 4,
            ...
            w: 8,
            h: tr.h,
            data: { kind: "region-edge", id: regionId, edge: "out", isOutput: false },
        };
    return {
        x: x1 + 4,
        y: tr.y,
        w: Math.max(0, x2 - x1 - 8),
        h: tr.h,
        data: { kind: "region", id: regionId, isOutput: false },
    };
}
...
export function outputRegionHit(
    snap: Snapshot,
    regionId: string,
    edge: "in" | "out" | "body" = "body",
): HitEntry {
    ...
    if (edge === "in")
        return {
            ...
            w: 8,
            h: tr.h,
            data: { kind: "region-edge", id: regionId, edge: "in", isOutput: true },
        };
    if (edge === "out")
        return {
            x: x2 - 4,
            ...
            w: 8,
            h: tr.h,
            data: { kind: "region-edge", id: regionId, edge: "out", isOutput: true },
        };
    return {
        x: x1 + 4,
        y: tr.y,
        w: Math.max(0, x2 - x1 - 8),
        h: tr.h,
        data: { kind: "region", id: regionId, isOutput: true },
    };
}
            ...
            switch (i.kind) {
                ...
                case "regionResize":
                    if (i.isOutput) {
                        store.dispatch(
                            commitClipoutResize({
                                id: i.id,
                                inBeatTime: i.inPoint,
                                outBeatTime: i.outPoint,
                                altKey: i.altKey,
                            }),
                        );
                    } else {
                ...
                case "regionEntityMove":
                    if (i.isOutput) {
                        ...
                        store.dispatch(
                            commitClipoutPan({ id: i.id, delta: i.delta, altKey: i.altKey }),
                        );
                    } else {
                ...
                case "beginDrag":
                    store.dispatch(
                        beginDragThunk({ handle: i.handle, pxPerUnit: i.pxPerUnit, grid: i.grid }),
                    );
                    break;
                case "drag":
...
function makeSnapFromState(
    ...
    return {
        ...
        selectedOrigAnchorIds:
            overrides.selectedOrigAnchorIds ?? new Set(state.warp.selectedOrigIds),
        selectedBeatAnchorIds:
            overrides.selectedBeatAnchorIds ?? new Set(state.warp.selectedBeatIds),
        regions:
            overrides.regions ??
            state.region.regions.map((r) => ({
                id: r.id,
                inPoint: r.inPoint,
                outPoint: r.outPoint,
            })),
        regionsOutput:
            overrides.regionsOutput ??
            state.region.regions.map((r) => ({
                id: r.id,
                inPoint: r.inBeatTime,
                outPoint: r.outBeatTime,
            })),
