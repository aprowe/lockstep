...
describeFeature(feature, ({ Scenario }) => {
  ...
  Scenario('Shift + wheel pans horizontally even when deltaY is 0', ({ Given, When, Then }) => {
    ...
    When('the user scrolls the mouse wheel while holding Shift', () => {
      ...
      intents = c.wheel(makeWheel({ clientX: 500, clientY: 200, shiftKey: true, deltaX: 0, deltaY: 200 }), snap)
    })
  ...
  Scenario('Ctrl/Cmd + wheel zooms around the cursor', ({ Given, And, When, Then }) => {
    ...
    const cursorX = 500
    const timeAtCursorBefore = snap.view.start + (cursorX / snap.canvas.width) * (snap.view.end - snap.view.start)
    ...
    When('the user scrolls the mouse wheel while holding Ctrl or Cmd', () => {
      intents = c.wheel(makeWheel({ clientX: cursorX, clientY: 200, ctrlKey: true, deltaX: 0, deltaY: -100 }), snap)
    })
    ...
    And('the time at horizontal position X stays at horizontal position X', () => {
      const v = findIntent(intents, 'viewChange')!
      const timeAtCursorAfter = v.view.start + (cursorX / snap.canvas.width) * (v.view.end - v.view.start)
  ...
  Scenario('Alt + click + drag pans the viewport', ({ Given, When, Then }) => {
    ...
    When('the user holds Alt and drags the timeline', () => {
      ...
      intents = c.pointerMove(makePointer({ clientX: 600, clientY: 200, altKey: true }), snap)
    })
  ...
  Scenario('Clicking the minimap recenters the viewport', ({ Given, When, Then, And }) => {
    const c = createTimelineController()
    let intents: Intent[] = []
    const snap = makeSnap({ view: { start: 0, end: 20 }, maxDuration: 100, duration: 100, outputDuration: 100 })
  ...
  Scenario('Zoom is clamped to a minimum span of 0.1 seconds', ({ Given, When, Then }) => {
    ...
    When('the user attempts to zoom in past the minimum span', () => {
      intents = c.wheel(makeWheel({ clientX: 500, clientY: 200, ctrlKey: true, deltaX: 0, deltaY: -5000 }), snap)
    })
  ...
  Scenario('Zoom is clamped to a maximum of twice the video duration', ({ Given, When, Then }) => {
    const c = createTimelineController()
    let intents: Intent[] = []
    const maxDuration = 100
    const snap = makeSnap({ view: { start: 40, end: 60 }, maxDuration, duration: maxDuration, outputDuration: maxDuration })
  ...
  Scenario('Viewport is always clamped to the video duration', ({ Given, When, Then }) => {
    ...
    let intents: Intent[] = []
    const maxDuration = 100
    const snap = makeSnap({ view: { start: 5, end: 25 }, maxDuration, duration: maxDuration, outputDuration: maxDuration })
    ...
    When('the viewport would extend before 0 or past the video duration', () => {
      ...
      intents = c.wheel(makeWheel({ clientX: 500, clientY: 200, deltaX: -10000, deltaY: 0 }), snap)
    })
