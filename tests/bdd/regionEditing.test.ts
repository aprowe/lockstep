...
describeFeature(feature, ({ Scenario, ScenarioOutline, BeforeEachScenario }) => {
    ...
    ScenarioOutline(
        "Clicking a region moves the playhead to its start",
        ({ Given, And, When, Then }, variables) => {
            ...
            When("the user clicks the region in the <surface>", () => {
                ...
                if (surface === "clip sidebar") {
                    ...
                    harness.seek.mockImplementation(() => {
                        observed.selected = region.id;
                    });
                    const row = harness.container.querySelector(
                        ".clip-row:not(.clip-row--full)",
                    ) as HTMLElement;
                    fireEvent.click(row);
                    return;
                }
    ...
    Scenario(
        ...
        ({ Given, When, Then, And }) => {
            ...
            Then(
                "the clip name becomes an inline editable input with the current name selected",
                () => {
