...
describeFeature(feature, ({ Scenario, BeforeEachScenario }) => {
    ...
    Scenario(
        "Export to a folder whose parents don't exist creates them",
        ({ Given, When, Then, And }) => {
    ...
    Scenario("User Exports PTS set Video (Default)", ({ Given, When, And, Then }) => {
        ...
        Then("my output video will run at with PTS set to control the variable speed", async () => {
            await waitFor(() => expect(warpApi.startWarp).toHaveBeenCalled());
    ...
    Scenario(
        "Batch export saves each clip to the destination as it finishes",
        ({ Given, And, When, Then }) => {
            ...
            Then(
                "each clip is written into the destination folder the moment its render completes",
                async () => {
            ...
            And(
                "the next clip's render does not have to finish before earlier clips are saved",
                () => {
    ...
    Scenario(
        "The {beats} token in the filename pattern resolves to the clip's beat count",
        ({ Given, And, When, Then }) => {
    ...
    Scenario(
        "Show Folder button appears on the progress screen as soon as export begins",
        ({ Given, When, Then, And }) => {
            ...
            And(
                "the button remains available for the rest of the export and after it finishes",
                async () => {
                    // Finish the in-flight render — the post-export view also exposes a folder button.
                    fireProgress({ job_id: "job-1", status: "done", output_path: "/tmp/a.mp4" });
                    await waitFor(() =>
                        expect(
                            screen.queryAllByRole("button", { name: /Folder/ }).length,
                        ).toBeGreaterThan(0),
                    );
                },
            );
