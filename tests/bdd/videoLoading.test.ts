...
describeFeature(feature, ({ Scenario, BeforeEachScenario }) => {
    ...
    Scenario("Viewport is set to the video duration on load", ({ When, Then }) => {
        When("a video is loaded", async () => {
            vi.mocked(videoApi.loadVideoFromPath).mockResolvedValue(
                makeVideoInfo({ duration: 240 }),
            );
    ...
    Scenario(
        "File browser is reachable before any video is loaded",
        ({ Given, When, Then, And }) => {
            const fakeEntries = [
