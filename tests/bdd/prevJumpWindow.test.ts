...
describeFeature(feature, ({ ScenarioOutline }) => {
    ...
    ScenarioOutline(
        "While playing, Previous skips targets within 0.5s behind the playhead",
    ...
    ScenarioOutline(
        "While playing, Previous still snaps to the nearest earlier target outside the 0.5s window",
        ({ Given, And, When, Then }, variables) => {
            const state = {
                playhead: 0,
                playing: false,
    ...
    ScenarioOutline(
        "While paused, the 0.5s window does not apply to Previous",
        ({ Given, And, When, Then }, variables) => {
            const state = {
                playhead: 0,
                playing: false,
